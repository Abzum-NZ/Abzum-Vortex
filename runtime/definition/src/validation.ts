import {
  ...
  PLATFORM_BLOCK_RELEASES_BY_ID,
  COMPONENT_SUPPORTED_EVENTS,
  REGISTERED_FLOW_NODE_KINDS,
  REGISTERED_FLOW_OUTCOMES,
...
const failure = (
  ...
  component?:
    | DefinitionValidationLocation["segments"][number]
    | readonly DefinitionValidationLocation["segments"][number][],
  ...
  return {
    ...
    location: {
      ...rootLocation(output),
      segments: [...rootLocation(output).segments, ...segments],
    },
  };
};
...
const sourceFailure = (
  ...
  ruleCode: string,
  family: DefinitionRuleFailure["family"],
  segments: readonly DefinitionValidationLocation["segments"][number][] = [],
): DefinitionRuleFailure => ({
  ruleCode,
  family,
  location: {
    ...
    segments: [{ kind: "application", key: source.key }, ...segments],
  },
});
...
function sourceTranslationContext(source: unknown) {
  ...
  const visit = (
    ...
    if (collectionName === "placements" && entry && typeof entry === "object" && !Array.isArray(entry)) {
      for (const [placementKey, placement] of Object.entries(entry)) {
        const parsedCandidate = namespacedKeySchema.or(builderKeySchema).safeParse(placementKey);
        const placementSegments = parsedCandidate.success
          ? [...segments, { kind: "block" as const, key: parsedCandidate.data }]
          : segments;
        pathMap.push({
          sourcePath: [...path, placementKey],
          location: { ...rootLocation, segments: placementSegments },
        });
        visit(placement, [...path, placementKey], placementSegments, "block");
      }
      return;
    }
    if (collectionName === "settings" && entry && typeof entry === "object" && !Array.isArray(entry)) {
      for (const [settingKey, settingVal] of Object.entries(entry)) {
        const parsedCandidate = namespacedKeySchema.or(builderKeySchema).safeParse(settingKey);
        const settingSegments = parsedCandidate.success
          ? [...segments, { kind: "setting" as const, key: parsedCandidate.data }]
          : segments;
        pathMap.push({
          sourcePath: [...path, settingKey],
          location: { ...rootLocation, segments: settingSegments },
        });
        visit(settingVal, [...path, settingKey], settingSegments, "setting");
      }
      return;
    }
  ...
  for (const output of context.outputs.filter((entry) => entry.kind === "application")) {
    ...
    for (const page of pages.values()) {
      ...
      if (page.type === "public") {
        ...
        for (const placement of placements) {
          ...
          const blockId = String(object(placement.block).blockId);
          const release = PLATFORM_BLOCK_RELEASES_BY_ID.get(blockId);
          if (release && release.capabilities.publicSurface !== "allowed") {
            publicBlockReferencesSafe = false;
          }
    ...
    for (const binding of array(content.flowBindings)) {
      ...
      if (
        ...
        const controlPlacement = applicationPlacements.get(String(binding.controlId));
        if (controlPlacement) {
          const blockId = String(object(controlPlacement.block).blockId);
          const release = PLATFORM_BLOCK_RELEASES_BY_ID.get(blockId);
          if (release) {
            const supportedEvents = COMPONENT_SUPPORTED_EVENTS[release.componentKey] ?? [];
            if (!supportedEvents.includes(binding.event as ComponentSemanticEventKind)) {
              failures.push(
                bindingFailure("vortex.definition.application_flow_binding_target", "unsupported_choice"),
              );
            }
          }
        }
...
const unsafePatterns: readonly RegExp[] = Object.freeze([
  /<\s*(?:script|iframe|object|embed|applet|style|meta|link|base|body|html|form|input|button|svg|math|img|video|audio|source|track|a|div|span|p|h[1-6]|table|tr|td|th|ul|ol|li)\b/i,
  /\bon[a-z]+\s*=/i,
  /(?:javascript|vbscript)\s*:/i,
  /data\s*:\s*text\/html/i,
  /<[A-Z][A-Za-z0-9]*\b/,
  /dangerouslySetInnerHTML\b/i,
  /\bclass(?:Name)?\s*=/i,
  /\bstyle\s*=/i,
  /\bstyle\s*:\s*["'{]/i,
  /(?:expression\s*\(|@import\b|-moz-binding\b|behavior\s*:)/i,
  /\b(?:SELECT\s+.*?\s+FROM|INSERT\s+INTO|UPDATE\s+.*?\s+SET|DELETE\s+FROM|DROP\s+(?:TABLE|DATABASE|VIEW|INDEX)|ALTER\s+TABLE|CREATE\s+(?:TABLE|DATABASE|VIEW)|EXEC(?:\s+|\()|UNION\s+SELECT)\b/i,
  /(?:rpc:\/\/|grpc:\/\/|jsonrpc\b)/i,
  /(?:window\.\w+\s*\(|document\.\w+\s*\()/i,
  /(?:(?:function\s*\(|\(\s*\)\s*=>|eval\s*\(|new\s+Function\b|require\s*\(|import\s*\(|\bprocess\.env\b))/i,
]);

function isUnsafeString(str: string): boolean {
...
function extractRichTextKinds(document: unknown): Set<string> {
  const kinds = new Set<string>();
  if (!document || typeof document !== "object") return kinds;
  const doc = document as { blocks?: unknown[] };
  if (!Array.isArray(doc.blocks)) return kinds;
  const visitInline = (inline: unknown): void => {
    if (!inline || typeof inline !== "object") return;
    const item = inline as { kind?: string; children?: unknown[] };
    if (item.kind && item.kind !== "text") kinds.add(item.kind);
    if (Array.isArray(item.children)) {
      for (const child of item.children) visitInline(child);
    }
  };
  for (const block of doc.blocks) {
    if (!block || typeof block !== "object") continue;
    const b = block as { kind?: string; children?: unknown[]; items?: unknown[] };
    if (b.kind) kinds.add(b.kind);
    if (Array.isArray(b.children)) {
      for (const child of b.children) visitInline(child);
    }
...
function validateSettingAgainstProperty(
  settingVal: unknown,
  schema: BlockPropertySchemaV2Contract,
  settingSegments: DefinitionValidationLocation["segments"],
  makeFailure: (
    ruleCode: string,
    family: DefinitionRuleFailure["family"],
    segments: DefinitionValidationLocation["segments"],
  ) => DefinitionRuleFailure,
  failures: DefinitionRuleFailure[],
): void {
  if (settingVal === null || typeof settingVal !== "object") {
    failures.push(
      makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
    );
    return;
  }
  const valObj = settingVal as {
    kind?: string;
    value?: unknown;
  ...
  const valKind = valObj.kind;

  switch (schema.kind) {
    case "text": {
      if (valKind !== "text" || typeof valObj.value !== "string") {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
        return;
      }
      const text = valObj.value;
    ...
    case "boolean": {
      if (valKind !== "boolean" || typeof valObj.value !== "boolean") {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
      }
    ...
    case "choice": {
      if (valKind !== "choice" || typeof valObj.value !== "string") {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
        return;
      }
      const allowedKeys = new Set(schema.options.map((opt) => opt.key));
      if (!allowedKeys.has(valObj.value)) {
        failures.push(
          makeFailure(
            "vortex.definition.application_block_settings",
            "unsupported_choice",
    ...
    case "rich_text": {
      if (valKind !== "rich_text" || !valObj.value) {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
        return;
      }
      const allowedElements = new Set(schema.allowedElements);
      const usedKinds = extractRichTextKinds(valObj.value);
      if ([...usedKinds].some((k) => !allowedElements.has(k as any))) {
        failures.push(
          makeFailure(
            "vortex.definition.application_block_settings",
            "unsupported_choice",
    ...
    case "group": {
      if (valKind !== "group" && valKind !== "object") {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
        return;
      }
      const subVals = (valObj.properties ?? {}) as Record<string, unknown>;
      for (const subProp of schema.properties) {
        if (subProp.required && !(subProp.key in subVals)) {
          failures.push(
            makeFailure(
              "vortex.definition.application_block_settings",
              "required_value",
        ...
        } else if (subProp.key in subVals) {
          validateSettingAgainstProperty(
            subVals[subProp.key],
            subProp,
            settingSegments,
            makeFailure,
    ...
    case "list": {
      if (valKind !== "list" || !Array.isArray(valObj.items)) {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
        return;
      }
      const items = valObj.items;
      if (schema.minimumItems !== undefined && items.length < schema.minimumItems) {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "too_few_items", settingSegments),
        );
      }
      if (schema.maximumItems !== undefined && items.length > schema.maximumItems) {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "too_many_items", settingSegments),
        );
      }
      for (const item of items) {
        validateSettingAgainstProperty(
    ...
    case "url": {
      if (valKind !== "url" || typeof valObj.value !== "string" || !valObj.value.startsWith("https://")) {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
      }
    ...
    case "icon": {
      if (valKind !== "icon" || typeof valObj.icon_key !== "string") {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
      }
    ...
    case "asset_reference": {
      if (valKind !== "asset_reference" || typeof valObj.asset_id !== "string") {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
      }
    ...
    case "theme_token": {
      if (valKind !== "theme_token" || typeof valObj.token !== "string") {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
      }
    ...
    case "field_reference": {
      if (valKind !== "field_reference" || !valObj.field) {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
      }
    ...
    case "relationship_reference": {
      if (valKind !== "relationship_reference" || !valObj.relationship) {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
      }
    ...
    case "action_reference": {
      if (valKind !== "action_reference" || typeof valObj.action !== "string") {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
      }
    ...
    case "page_reference": {
      if (valKind !== "page_reference" || typeof valObj.page !== "string") {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
      }
    ...
    case "query_reference": {
      if (valKind !== "query_reference" || typeof valObj.query !== "string") {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
      }
    ...
    case "pipeline_reference": {
      if (valKind !== "pipeline_reference" || typeof valObj.pipeline !== "string") {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "invalid_value", settingSegments),
        );
      }
...
function extractApplicationSourcePlacements(
  source: ApplicationSourceDocumentV2,
): { placementAlias: string; placement: SourceBlockPlacementV2; isPublicPage: boolean }[] {
  const result: { placementAlias: string; placement: SourceBlockPlacementV2; isPublicPage: boolean }[] = [];
  for (const shell of source.body.shells) {
    for (const [alias, placement] of sourcePlacementEntriesV2(shell.layout)) {
      result.push({ placementAlias: alias, placement, isPublicPage: false });
    }
  }
  for (const page of source.body.pages) {
    const isPublic = page.type === "public";
    const comp = page.composition;
    if ("step_content" in comp) {
      if (comp.shell_kind === "default") {
        for (const slot of Object.values(comp.step_content)) {
          for (const [alias, placement] of sourcePlacementEntriesV2(slot)) {
            result.push({ placementAlias: alias, placement, isPublicPage: isPublic });
          }
        }
      } else {
        for (const content of Object.values(comp.step_content)) {
    ...
    } else if (comp.shell_kind === "default") {
      for (const [alias, placement] of sourcePlacementEntriesV2(comp.main)) {
        result.push({ placementAlias: alias, placement, isPublicPage: isPublic });
      }
    } else {
      for (const slot of Object.values(comp.content)) {
        for (const [alias, placement] of sourcePlacementEntriesV2(slot)) {
          result.push({ placementAlias: alias, placement, isPublicPage: isPublic });
        }
      }
...
export function validateApplicationSourceCatalogue(
  ...
  // 1. Platform block dependencies manifest
  const manifestMap = new Map<
    string,
    { block_id: string; release_version: string; content_fingerprint: string }
  >();
  for (const dep of source.body.platform_block_dependencies) {
    const blockId = String(dep.block_id);
    const release = PLATFORM_BLOCK_RELEASES_BY_ID.get(blockId);
    if (!release) {
      failures.push(
        makeFailure("vortex.definition.application_dependency_manifest", "broken_reference"),
      );
    } else if (release.releaseVersion !== dep.release_version) {
      failures.push(
        makeFailure("vortex.definition.application_dependency_manifest", "incompatible_version"),
      );
    } else if (dep.content_fingerprint && release.contentFingerprint !== dep.content_fingerprint) {
      failures.push(
        makeFailure("vortex.definition.application_dependency_manifest", "incompatible_version"),
      );
    }
    manifestMap.set(blockId, dep);
  }

  // Extract all placements
  const placementEntries = extractApplicationSourcePlacements(source);
  const placementMap = new Map<string, SourceBlockPlacementV2>();
  const usedBlockIds = new Set<string>();

  // Check composition policy limits
  if (
    placementEntries.length >
    IMMUTABLE_PLATFORM_BLOCK_CATALOGUE_V2.compositionPolicy.maximumPlacements
  ) {
    failures.push(
      makeFailure("vortex.definition.application_layout_complete", "too_many_items"),
    );
  }

  // 2. Validate placements against catalogue and manifest
  const seenPlacements = new Set<string>();
  for (const { placementAlias, placement, isPublicPage } of placementEntries) {
    if (seenPlacements.has(placementAlias)) continue;
    seenPlacements.add(placementAlias);

    placementMap.set(placementAlias, placement);
    const blockRef = placement.block;
    const blockId = String(blockRef.block_id);
    usedBlockIds.add(blockId);
    const release = PLATFORM_BLOCK_RELEASES_BY_ID.get(blockId);
    const blockSegments: DefinitionValidationLocation["segments"] = [
      { kind: "block", key: placementAlias },
    ];

    // Manifest match for placement
    ...
    if (!manifestEntry) {
    ...
    } else if (manifestEntry.release_version !== blockRef.release_version) {
      failures.push(
        makeFailure(
          "vortex.definition.application_dependency_manifest",
      ...
      blockRef.content_fingerprint &&
      manifestEntry.content_fingerprint !== blockRef.content_fingerprint
    ) {
      failures.push(
        makeFailure(
          "vortex.definition.application_dependency_manifest",
    ...
    // Release existence and version match
    if (!release) {
      failures.push(
        makeFailure(
          "vortex.definition.application_block_references",
    ...
    if (
      release.releaseVersion !== blockRef.release_version ||
      (blockRef.content_fingerprint && release.contentFingerprint !== blockRef.content_fingerprint)
    ) {
      failures.push(
        makeFailure(
          "vortex.definition.application_block_references",
    ...
    // 3. Public surface safety
    if (isPublicPage && release.capabilities.publicSurface !== "allowed") {
      failures.push(
        makeFailure(
          "vortex.definition.application_public_surface",
          "unsafe_content",
    ...
    // 4. Responsive capabilities
    if (placement.responsive) {
      if (
        placement.responsive.hide_on_mobile === true &&
        release.capabilities.responsive.mobileHidden !== true
      ) {
        failures.push(
          makeFailure(
            "vortex.definition.application_layout_complete",
      ...
      if (
        placement.responsive.collapse_to_drawer_on_mobile === true &&
        release.capabilities.responsive.drawerCollapsible !== true
      ) {
        failures.push(
          makeFailure(
            "vortex.definition.application_layout_complete",
    ...
    // 5. Accessible name requirement
    const declaredProps = new Map(release.properties.map((p) => [p.key, p]));
    const settings = (placement.settings ?? {}) as Record<string, unknown>;

    if (release.capabilities.accessibleName === "required") {
      const explicitName = (placement as any).accessible_name;
      let hasAccessibleName =
        typeof explicitName === "string" && explicitName.trim().length > 0;
      if (
        !hasAccessibleName &&
        release.capabilities.accessibleNamePropertyPath &&
        release.capabilities.accessibleNamePropertyPath.length > 0
      ) {
        const propKey = release.capabilities.accessibleNamePropertyPath[0]!;
        const propVal = settings[propKey] as { kind?: string; value?: unknown } | undefined;
        if (
          propVal &&
          propVal.kind === "text" &&
          typeof propVal.value === "string" &&
          propVal.value.trim().length > 0
        ) {
    ...
    // 6. Settings schema & unsafe content
    for (const key of Object.keys(settings)) {
      if (!declaredProps.has(key)) {
        failures.push(
          makeFailure("vortex.definition.application_block_settings", "unknown_property", [
            ...blockSegments,
            { kind: "setting", key },
          ]),
        );
    ...
    for (const prop of release.properties) {
      const settingVal = settings[prop.key];
      const settingSegments: DefinitionValidationLocation["segments"] = [
        ...blockSegments,
        { kind: "setting", key: prop.key },
      ];
      if (prop.required && settingVal === undefined) {
        failures.push(
          makeFailure(
            "vortex.definition.application_block_settings",
            "required_value",
      ...
      } else if (settingVal !== undefined) {
        if (hasUnsafeContent(settingVal)) {
          failures.push(
            makeFailure(
              "vortex.definition.application_block_settings",
              "unsafe_content",
    ...
    // 7. Child slots validation
    const declaredSlots = new Map(release.slots.map((s) => [s.key, s]));
    const placementSlots = (placement.slots ?? {}) as Record<string, SourcePlacementSlotV2>;

    for (const slotKey of Object.keys(placementSlots)) {
    ...
    for (const slotDecl of release.slots) {
      const slot = placementSlots[slotDecl.key];
      if (slotDecl.required && (!slot || Object.keys(slot.placements).length === 0)) {
        failures.push(
          makeFailure(
            "vortex.definition.application_block_references",
            "required_value",
      ...
      if (slot) {
        const childPlacementEntries = Object.entries(slot.placements);
        if (
          slotDecl.maximumChildren !== undefined &&
          childPlacementEntries.length > slotDecl.maximumChildren
        ) {
          failures.push(
            makeFailure(
              "vortex.definition.application_block_references",
        ...
        const allowedCategories = new Set(slotDecl.allowedChildCategories);
        for (const [, childPlacement] of childPlacementEntries) {
          const childBlockId = String(childPlacement.block.block_id);
          const childRelease = PLATFORM_BLOCK_RELEASES_BY_ID.get(childBlockId);
          if (childRelease && !allowedCategories.has(childRelease.paletteGroup)) {
            failures.push(
              makeFailure(
                "vortex.definition.application_block_references",
                "unsupported_choice",
  ...
  // Unused block dependencies check
  for (const dep of source.body.platform_block_dependencies) {
    if (!usedBlockIds.has(String(dep.block_id))) {
      failures.push(
        makeFailure("vortex.definition.application_dependency_manifest", "scope_conflict"),
      );
    }
  }

  // 8. Flows validation
  const flowsByAlias = new Map(source.body.flows.map((f) => [f.id, f]));
  for (const flow of source.body.flows) {
    const flowFailure = (code: string, fam: DefinitionRuleFailure["family"]) =>
      makeFailure(code, fam, [{ kind: "flow", key: flow.key }]);
    const nodes = flow.nodes;
    const edges = flow.edges;
    const byId = new Map(nodes.map((n) => [n.id, n]));

    for (const node of nodes) {
      if (!REGISTERED_FLOW_NODE_KINDS.includes(node.kind as any)) {
        failures.push(
          flowFailure("vortex.definition.application_flow_node_references", "broken_reference"),
        );
      }
    }
    ...
    if (starts.length !== 1) {
      failures.push(
        flowFailure("vortex.definition.application_flow_single_start", "invalid_value"),
      );
    }
    ...
    for (const edge of edges) {
      ...
      if (edge.outcome !== undefined && !REGISTERED_FLOW_OUTCOMES.includes(edge.outcome as any)) {
        failures.push(
          flowFailure("vortex.definition.application_flow_node_values", "invalid_value"),
        );
      }
    }
    if (!edgeEndpointsValid) {
      failures.push(
        flowFailure("vortex.definition.application_flow_edge_endpoints", "broken_reference"),
      );
    }
    ...
    if (starts.length === 1) {
      ...
      if (
        (incoming.get(startId)?.length ?? 0) !== 0 ||
        (nodes.length > 1 && (outgoing.get(startId)?.length ?? 0) < 1)
      ) {
        failures.push(
          flowFailure("vortex.definition.application_flow_single_start", "invalid_value"),
        );
      ...
      if (reachable.size !== nodes.length) {
        failures.push(
          flowFailure("vortex.definition.application_flow_reachability", "broken_reference"),
        );
      }
    }
    ...
    if (returns.length < 1) {
      failures.push(
        flowFailure("vortex.definition.application_flow_termination", "invalid_value"),
      );
    } else {
      for (const ret of returns) {
        if ((outgoing.get(ret.id)?.length ?? 0) > 0) {
          failures.push(
            flowFailure("vortex.definition.application_flow_termination", "scope_conflict"),
          );
          break;
        }
      }
      const canTerminate = new Set(returns.map((n) => n.id));
      let termChanged = true;
      while (termChanged) {
        termChanged = false;
        for (const edge of edges) {
          if (canTerminate.has(edge.to_node) && !canTerminate.has(edge.from_node)) {
            canTerminate.add(edge.from_node);
            termChanged = true;
          }
        }
      }
      if (nodes.some((n) => !canTerminate.has(n.id))) {
        failures.push(
          flowFailure("vortex.definition.application_flow_termination", "dependency_cycle"),
        );
      }
    }

    const inDegrees = new Map<string, number>();
    for (const node of nodes) inDegrees.set(node.id, incoming.get(node.id)?.length ?? 0);
    const zeroInQueue: string[] = [];
    for (const [id, deg] of inDegrees.entries()) {
      if (deg === 0) zeroInQueue.push(id);
    }
    ...
    if (processedCount !== nodes.length) {
      failures.push(
        flowFailure("vortex.definition.application_flow_acyclic", "dependency_cycle"),
      );
    }
  }

  // 9. Flow bindings validation
  for (const binding of source.body.flow_bindings) {
    const bindingFailure = (code: string, fam: DefinitionRuleFailure["family"]) =>
      makeFailure(code, fam, [{ kind: "flow_binding", key: binding.id }]);
    const placement = placementMap.get(binding.control);
    if (!placement) {
      failures.push(
        bindingFailure("vortex.definition.application_flow_binding_target", "broken_reference"),
      );
    ...
    if (binding.flow.kind === "application_owned") {
      ...
      if (!flow) {
        ...
        const declaredInputs = flow.inputs ?? {};
        const inputs = binding.inputs ?? {};
        for (const [key, inputDecl] of Object.entries(declaredInputs)) {
          const isRequired =
            (inputDecl as any).required !== false && (inputDecl as any).defaultValue === undefined;
          if (isRequired && !(key in inputs)) {
            failures.push(
              bindingFailure(
                "vortex.definition.application_flow_binding_inputs",
        ...
        for (const [key, boundInput] of Object.entries(inputs)) {
          ...
          if (
            boundInput.type &&
            decl.type &&
            !flowTypesCompatible(String(boundInput.type), String(decl.type))
          ) {
            failures.push(
              bindingFailure(
                "vortex.definition.application_flow_binding_inputs",
          ...
          if (
            boundInput.value.source === "literal" &&
            !flowLiteralMatchesType((boundInput.value as any).value, String(boundInput.type))
          ) {
            failures.push(
              bindingFailure(
                "vortex.definition.application_flow_binding_inputs",
...
function applicationCatalogueValidationRule(
  context: PreparedValidationContext,
): DefinitionRuleFailure[] {
  const failures: DefinitionRuleFailure[] = [];
  for (const [index, raw] of editSaveSources(context).entries()) {
    const parsed = context.parsedSources?.[index] ?? parseEditSaveSource(raw);
    if (!parsed.success || parsed.data.kind !== "application") continue;
    if (isV2ApplicationSource(parsed.data)) {
      failures.push(
