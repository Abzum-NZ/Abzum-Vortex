...
const unsafeTextPatterns: readonly RegExp[] = Object.freeze([
  ...
  /\bnew\s+Function\s*\(/i,
  /\b(?:document|window)\s*\.\s*(?:cookie|write|writeln|location|open|eval)\b/i,
  /\([\w\s,]*\)\s*=>\s*\{/,
  /\bdangerouslySetInnerHTML\b/i,
  // Arbitrary CSS and class names.
  /\b(?:style|class|className)\s*=\s*["'{]/i,
  /@(?:import|media|font-face|keyframes|charset|namespace)\b/i,
  /\bexpression\s*\(/i,
  /\{\s*[a-z-]+\s*:\s*[^;{}]+;/i,
  // SQL statements.
  ...
  /\bcreate\s+(?:or\s+replace\s+)?(?:table|function|procedure|view)\s+[\w."]+\s*(?:\(|as\b)/i,
  /\bexec(?:ute)?\s+(?:procedure|function|immediate)\b/i,
