...
const validateAccessibleName = (
  ...
  const contract = release.capabilities.accessibleName;
  if (contract.requirement === "not_applicable") return;
  let current: BlockPropertyValueV2Contract | undefined;
  let properties = settings;
  for (const [index, key] of contract.propertyPath.entries()) {
    current = properties[key];
    if (current === undefined) break;
    if (index < contract.propertyPath.length - 1) {
  ...
  if (current.kind !== "text" || current.value.trim().length === 0)
    reject("vortex.definition.application_block_settings", "invalid_value");
};
