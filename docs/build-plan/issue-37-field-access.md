...
1. Resolve current field permission declarations and #36's local-share bounds into separate readable and changeable sets. Reuse the existing declaration/permission identities, not hardcoded field names, role labels or another access store.
   ...
   Before using a final result, match its exact record identifier, organisation/application/type/storage binding, operation and current transaction evidence to the requested record. An allowed result for another record of the same type grants nothing here; reuse #35's existing evidence rather than creating another token or cache.
