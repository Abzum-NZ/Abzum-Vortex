...
The supported database publication path must enforce the same ownership rule before it stores a new immutable release. A Module validates against its own candidate release. An Application validates its own permissions only against the exact Module release revisions in its sealed dependency manifest; it never consults a newer or merely available Module release. Reuse the canonical policy-shape validator, keep registration as an exact consumer of sealed release evidence, and do not repeat the rule in each runtime consumer. A refused append is atomic, including its dependency rows and current-release pointer. Existing immutable releases with omitted policies remain readable and supply no field authority. This boundary is tracked by [#396](https://github.com/Abzum-NZ/Abzum-Vortex/issues/396).
