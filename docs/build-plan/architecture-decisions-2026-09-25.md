...
- **Theme catalogue.** Every option offered by [shadcn/create](https://ui.shadcn.com/create) at the pinned shadcn version is imported into the platform theme catalogue: visual style (Vega, Nova, Maia, Lyra, Mira, Luma, Sera, Rhea and any later style), base colour, theme colour, chart colour, heading font, body font, icon library, radius, menu and menu accent. An application theme selects one value per dimension and may override individual tokens. A shadcn preset code or preset URL can be imported into those selections. The catalogue is generated at build time from the pinned registry; nothing is fetched at run time, and fonts and icons are self-hosted.
- **Styles at run time.** In the Base UI registry, components carry `cn-*` classes and each style is a CSS file. Vortex ships every style's CSS scoped to a `data-vortex-style` attribute and loads only the selected one, so one component source serves every style and a theme change needs no code change.
