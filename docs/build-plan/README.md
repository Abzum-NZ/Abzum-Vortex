...
**Foundation order:** Development, Testing, and Production use one operated Kestra instance; the plan does not require separate instances or assume that a new Development credential has been provisioned. Its environment namespaces, identities, webhook keys, Doppler configurations, target credentials and approval gates remain distinct, providing logical and credential separation rather than strong process isolation. Named operators may restart or redeploy that shared instance when needed after [#271](https://github.com/Abzum-NZ/Abzum-Vortex/issues/271) establishes fresh, consistent and recoverable shared-service state. Before application-workflow execution [#76](https://github.com/Abzum-NZ/Abzum-Vortex/issues/76), [#198](https://github.com/Abzum-NZ/Abzum-Vortex/issues/198) must independently test and align the pinned Kestra release and its state-database major on an officially supported combination before an upgrade. The change requires a verified off-host backup, a disposable restore rehearsal and a forward migration; an existing PostgreSQL data directory is never opened by an older major. Shared-instance operating authority does not approve Production database delivery, which retains its separate named approval and evidence gate.
