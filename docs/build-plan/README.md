...
- Hosted migrations leave `vortex_runtime` passwordless. Before protected traffic, an operator assigns a separate high-entropy password to the exact environment role, stores only the complete restricted transaction-pooler URL and public verification values in that environment's Vercel-synchronised Doppler root config, verifies the exact sync target, and redeploys the intended commit. Testing evidence refuses missing values and any project-owner, migration, general `DATABASE_*` or Kestra credential in the Vercel target, then proves one real protected identity journey. Production repeats this separately under its Production gate.
