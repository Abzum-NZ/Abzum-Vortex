...
The [#266](https://github.com/Abzum-NZ/Abzum-Vortex/issues/266) bootstrap is deployed. Its pooled-connection observer repair passed in hosted Testing after [PR #297](https://github.com/Abzum-NZ/Abzum-Vortex/pull/297), together with all 30 SQL suites and the first ten concurrency proofs. That run then exposed an assignment-expiry test window sampled too early for remote setup. Sample the expiry after the holder has acquired its lock, retain proof that the competing request waits before expiry and refuses after expiry, and rerun the current protected revision's complete gate. A successful bootstrap deployment or partial test run is not a successful delivery receipt. These bounded proof repairs require no further Kestra deployment and do not hold independent [#33](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33) work; see the [dated evidence](../evidence/issue-266/README.md#hosted-follow-up--permission-proof-passed-assignment-expiry-setup-repair).
