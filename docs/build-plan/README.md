...
**Invitation integration delivery:** After stewardship, [#33](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33) adds immutable pending membership/direct-assignment intent and a private all-or-nothing acceptance composition. Preserve account-only acceptance, but make its existing runtime route refuse pending intent before Identity mutation so it cannot omit intended access. Verify exact role/Group/window facts, one Access increment, unchanged replay and competing acceptance; no extra approval store or copied account/secret. [#40](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40) owns the verified-identity pre-organisation entry point and current inviter/approver authority checks; [#267](https://github.com/Abzum-NZ/Abzum-Vortex/issues/267) owns the human journey. This does not require the invitee to possess the account being created. See the [invitation contract](../specification/appendices/data-contracts.md#invitation).
