...
Current implementation is the [role-assignment/delegation ledger within #40](issue-40-protected-access-administration.md#next-safe-read-checkpoint-role-assignments-and-delegation-authorities). In parallel, [#36](issue-36-ownership-and-visibility.md) is completing the design for its remaining current ownership, Group, local-share and relationship row restrictions before the next bounded implementation. These are actual dependencies of later complete row/field enforcement, not optional UI work.
