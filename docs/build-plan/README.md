...
**Status:** Approved build plan 2.19
...
First close the database-free assignment/activation contract gap within #33: require explicit standing/eligible assignment kind, add the permanent activation identity, and distinguish direct from Group-derived per-account activation evidence with mandatory finite expiry. Prove strict source, policy, revision, time and revocation shapes before adding SQL storage. Timing-only status never establishes permission; successful parsing is not verified eligibility or approval. This slice adds no evaluator, runtime adapter, writer, scheduled activation or granting screen. Later storage retains permanent same-organisation source identities while comparing historical source revisions with mutable current facts, rather than blocking source changes through revision foreign keys.

Use the [initial membership lifecycle](../specification/appendices/groups-and-privileged-access.md#current-state-checks-and-revocation): removal/suspension is one inactive state; explicit same-identity restoration advances its revision and cannot revive old activations. Revoked role assignments and delegation require a new grant identity, and retired Groups do not resume. Verify these distinct transitions in storage/writer tests without inventing a separate suspension state or editing time bounds to imitate suspension.

The next storage/read slice adds five private current-fact tables and exact-ID typed readers for Groups, memberships, assignments, individual activations and delegation. Keep grant windows fixed: restore a revoked membership before its original expiry, preserving future-start or permanent timing; renew an expired membership with a new identity after closing its predecessor. One non-revoked membership occupies each organisation/Group/account pair. Activation creation validates current role/policy/eligibility and finite source-bounded duration, but storage is not a caller-authority or approval decision. Keep historical source revision numbers independent from permanent identity foreign keys so legitimate changes remain possible.

Use a single delegation row with a closed exact bounded-permission JSON set and whole-set replacement, validated against immutable catalogue evidence and current continuity. Its fingerprint is not authority. Private reads return zero or one fact and timing-only state, without a runtime adapter, public grant, broad snapshot or another evaluator. Verify isolation, lifecycle/revision constraints, current source integrity, exact tuples, typed reads and private privileges; then complete the remaining protected writers, Access-version atomicity, stewardship and invitation compositions. Follow the [current-fact storage boundary](../specification/appendices/data-contracts.md#permission-and-role-contracts); passing storage fixtures never substitutes for those later operations.

After the current-fact slice, first implement private role-assignment grant/revoke composition with exact reviewed role revision, fixed windows and one atomic Access-version increment. Then implement explicit custom-role changes and supplied-role acceptance against the complete affected-assignment manifest. This order supplies the actual competing assignment writer for role-change concurrency tests. Role-mode changes and permission broadening cannot race a new grant into unreviewed authority. Keep both compositions owner-only until stewardship, the central decision and the protected invocation boundary are complete; neither step is a usable IAM journey.

Before explicit role changes, add the documented `role_catalogue_changed` Access-version reason through the current and stored contracts and an additive database change, preserving historical Group-name compatibility. Define candidate and affected-assignment fingerprints with the existing canonical JSON/fingerprint helpers; fingerprints identify exact reviewed content, never replace relational validation or caller authority. Reuse one manifest for combined permission broadening and standing/activation-required mode changes; same-mode activation-policy detail changes retain compatible eligibility without requiring a separate manifest or regrant.

Build the explicit role changes as one private composition with closed operation variants. Keep [local metadata/policy editing separate from permission acceptance](../specification/appendices/groups-and-privileged-access.md#editing-a-role-versus-accepting-permissions): edits and retirement preserve the accepted snapshot even when its source is unavailable; current-template acceptance and custom permission replacement recheck current evidence. First fix the canonical command/evidence contracts, then implement and verify the atomic writer with the real assignment writer competing against affected-assignment review. Prove exact command/result pairing through the test-only hand-off, including newly created versus reused policy evidence; do not expose an owner-credential runtime adapter. Retired roles cannot return; exact same-state requests refuse without another revision. After role changes, complete the remaining Group, membership, activation and delegation changes before stewardship and invitation integration. Completing role writers alone does not close #33 or expose IAM administration.
...
Before completing the explicit role-change slice, verify the shared [Access-version observation-time rule](../specification/appendices/data-contracts.md#permission-and-role-contracts): a request waiting behind another change and a backwards clock observation still produce exactly one ordered version increment with a nondecreasing timestamp. Keep the timestamp safeguard and prove deterministic failure of the old behaviour, the corrected queued update, unchanged refusal/exhaustion rollback and the full competing role/assignment/application-change suite. Correct the shared writer through an additive migration; do not rewrite applied migrations or change eligibility/expiry clocks.

Apply the [proportionate-implementation rule](../specification/appendices/core-contract-boundary.md#keep-the-implementation-proportionate) throughout these slices. Keep one atomic, revision-checked role change, explicit acceptance of broadened/restored authority and clean competing-write refusal. Do not add further counters, fingerprints, approval stages or frameworks without a concrete uncovered failure case. Review simplification opportunities before starting the next slice; Local clock repair is not an IAM feature or a reason to expand its model.

The full membership verification exposed the same audit-observation defect in the already delivered permission-registration writers. Complete [#295](https://github.com/Abzum-NZ/Abzum-Vortex/issues/295) as a bounded additive correction to those two writers: observe after locking, preserve exact current/immutable revision evidence, and rerun the existing full gate. This is a technical prerequisite for verified completion of [#33](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33), not a user decision or clock-maintenance project.

After explicit role changes, implement [Group creation, label changes and terminal retirement](../specification/appendices/groups-and-privileged-access.md#creating-renaming-and-retiring-groups) as one small private composition over the existing Group facts. Reuse Group revisions, governance locking and the existing Group-related Access reason; add no new counter, manifest, fingerprint or evaluator. Verify preserved child facts and the real assignment-versus-retirement race. Membership add/remove/restore/renew follows separately: preserve fixed original windows, restore only before expiry, and renew an expired non-revoked predecessor by closing it and creating a distinct successor atomically with one Access change. Verify duplicate/competing changes and current source/time rechecks without another lifecycle engine. Then complete individual activation and delegation changes, stewardship and invitation integration. Root-cause fixes and focused behavioural tests take precedence over extra guard layers or unrelated maintenance.

Implement individual activation separately from delegation. Reuse the existing exact role/source facts and one governance transaction: activate with explicit requested duration and server-bound beneficiary, or terminally revoke an exact reviewed activation. Derive immutable policy/authority evidence and source-bounded expiry after locks; preserve independent windows and source-loss revocation. Add only the truthful `role_activation_changed` reason and correct the existing insertion validator's statement-start equality so post-lock activation is possible while expired sources still refuse. No new approval store, counter, evaluator or public writer. Verify the distinct duration bounds and actual competing role/assignment/membership changes, then complete delegation, stewardship and invitations. See the [activation scope](../specification/appendices/groups-and-privileged-access.md#activation-and-immediate-loss-of-access) and [#33](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33).

Delegation now has its own locally verified three-operation private composition: grant, replace the whole scope, and terminally revoke. It reuses the existing exact account/Group holder, fixed windows, bounded permission tuples, catalogue/continuity validation and canonical fingerprint utility. Replacement validates the proposed scope without requiring stale old authority to become current; revocation remains possible after source loss or expiry. Each successful change advances Access once with `delegation_changed`. No new evaluator, history, approval store or affected-assignment manifest is added. The [C3f evidence](../evidence/issue-33-delegation/README.md) records the distinct scope/lifecycle outcomes and real competing catalogue/Group changes. Next add permanent stewardship and invitation integration. Initial stewardship grants only the minimum platform powers; ongoing qualification permits separately accepted extra authority in the same direct permanent assignment, without combining incomplete assignments or accounts. [#34](https://github.com/Abzum-NZ/Abzum-Vortex/issues/34) supplies current authority and [#40](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40) checks before-and-after/onward delegation inside protected invocation; [#267](https://github.com/Abzum-NZ/Abzum-Vortex/issues/267) owns the human journey. See the [delegation specification](../specification/04-access-and-permissions.md#managing-access-is-different-from-using-data).

**Permanent stewardship delivery:** In [#33](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33), first deliver explicit adoption and the core safeguard (D1). [Provisioning #30](https://github.com/Abzum-NZ/Abzum-Vortex/issues/30) owns account/identity creation in the outer transaction; the private Access appointment receives an already-active account and derives the fixed minimum platform powers, direct permanent assignment and separate catalogue delegation. Reuse the governance lock, current catalogue and one `stewardship_changed` Access increment. Keep one current adoption requirement with immutable original provenance, not an owner bypass or new permission snapshot. Exact replay checks that some current steward still qualifies; it does not require the original administrator to remain active or recreate their former grants. In D1, wrap only the existing role, assignment, delegation and account-state writers with the final assertion. Future identity-projection lifecycle and organisation reactivation in #30 must also preserve the invariant under consistently ordered organisation locks. Prove last-administrator protection and competing removals using the actual writers, without a speculative historical-state framework.

D1 now has [local implementation and verification evidence](../evidence/issue-33-stewardship/README.md): explicit appointment, current qualification, safe replay after replacement, and the four existing Access-owned mutation safeguards. This is not hosted verification or completion of #33. Keep SQL acceptance and separate-session concurrency suites sequential on a shared database so their fixtures do not overlap.

The exact management-application requirement (D2) now has [local implementation and verification evidence](../evidence/issue-33-management-application/README.md). It reuses the same current requirement and existing sealed role entries. Binding activation/replacement is revision-checked and verifies an actual permanent steward's operating assignment; continuously accepted required permissions remain usable while unrelated additions await acceptance. The safeguard participates in actual application-access changes. Installation and the human journey remain [#64](https://github.com/Abzum-NZ/Abzum-Vortex/issues/64) and [#267](https://github.com/Abzum-NZ/Abzum-Vortex/issues/267), not a claim that private stewardship facts create an interface. Invitation integration is next; the sole effective-access decision and authorised invocation remain [#34](https://github.com/Abzum-NZ/Abzum-Vortex/issues/34) and [#40](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40). See the [stewardship contract](../specification/appendices/data-contracts.md#permission-and-role-contracts) and [setup/removal behaviour](../specification/appendices/iam-application.md#setup-and-removal).

**Invitation integration delivery:** After stewardship, [#33](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33) adds immutable pending membership/direct-assignment intent through one private create-with-intent composition over the existing Identity creator, then a separate all-or-nothing acceptance composition. Do not add a bind-existing-invitation API or issued-state mechanism. Preserve account-only acceptance, but make its existing runtime route refuse pending intent before Identity mutation so it cannot omit intended access. All paths lock governance before the invitation; first acceptance rechecks expiry after the wait. Verify exact role/Group/window facts, one `invitation_access_accepted` Access increment, unchanged replay and competing acceptance; no extra approval store or copied account/secret. [#40](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40) owns the verified-identity pre-organisation entry point, exact trusted workflow/intent linkage and current inviter/approver authority checks; [#267](https://github.com/Abzum-NZ/Abzum-Vortex/issues/267) owns the human journey. This does not require the invitee to possess the account being created. See the [invitation contract](../specification/appendices/data-contracts.md#invitation).
...
- [Row-policy composition #35](issue-35-row-policy-composition.md) follows [ownership and visibility #36](https://github.com/Abzum-NZ/Abzum-Vortex/issues/36), which follows the Activity foundation. Phase 3 proves the complete operation-permission plus row-visibility decision on controlled neutral tables through actual non-owner roles. [#45](https://github.com/Abzum-NZ/Abzum-Vortex/issues/45) later installs that policy shape on generated Definition-derived tables; do not create a circular dependency on Phase 4 storage or treat permission eligibility alone as row authority.
...
- [Reusable transactional Activity append #252](issue-252-activity-foundation.md) and headless entitlement policy/reservations [#118](https://github.com/Abzum-NZ/Abzum-Vortex/issues/118) before their first consuming operations. #252 is independently ready after #28/#34 and builds one owner-only, content-free append boundary with transaction/retry proofs; #115 later adds permitted views and full service coverage. Its immediate consumers are [ownership/visibility #36](https://github.com/Abzum-NZ/Abzum-Vortex/issues/36) and [protected role/Group administration #40](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40). It does not open a general append endpoint or introduce another runtime service.
