...
**Agent coordination — 12 September 2026:** GPT-6 Astra at Medium owns BA review, correction
...
architecture; GPT-5.6 Terra/Sol implement assigned work under the current user instruction. Follow the [coordination rules](agent-coordination.md)
...
The [App Designer HTML prototype](app-designer-html-prototype.md) and production designer follow the [complete file-defined application proof](definition-first-application-proof.md). Preserve the existing prototype during engine delivery. Record visibility [#36](https://github.com/Abzum-NZ/Abzum-Vortex/issues/36), access administration [#40](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40), row enforcement [#35](https://github.com/Abzum-NZ/Abzum-Vortex/issues/35), field access [#37](https://github.com/Abzum-NZ/Abzum-Vortex/issues/37) and page capability projection [#38](https://github.com/Abzum-NZ/Abzum-Vortex/issues/38) are Done. Remaining access work includes [#41](https://github.com/Abzum-NZ/Abzum-Vortex/issues/41) refusal Activity and the later query-cache integration [#39](https://github.com/Abzum-NZ/Abzum-Vortex/issues/39). Continue from the actual dependencies below; historical rejected candidates are not implementation instructions. The eventual designer and MCP authoring use the same protected definition operations and proven runtime.
...
10. Independent review and normal required checks precede merging to `testing`. **During a phase, a task is marked Done on local developer verification**: the repository gate (`pnpm verify`) plus the database checks the change affects, each on a fresh verification cluster, together with independent review where the change touches security, privileged database objects or concurrency. **Hosted verification is a phase gate, not a per-task step.** At the end of a phase the exact current Testing revision is deployed to `vortex-testing.abzum.com` and the required verification runs there once, covering the phase's accumulated work and selecting only the checks those changes affect. Track and resolve any failure before the phase closes. Production deployment is reserved for final project delivery. This policy does not claim that earlier deliveries met the previous per-migration receipt rule or the previous per-task hosted rule.
...
**Phase exit and follow-ups:** adviser cleanup [#235](https://github.com/Abzum-NZ/Abzum-Vortex/issues/235) and [#266](https://github.com/Abzum-NZ/Abzum-Vortex/issues/266)'s corrected Testing gate are complete. [#29](https://github.com/Abzum-NZ/Abzum-Vortex/issues/29)'s remaining open prerequisite is [#30](https://github.com/Abzum-NZ/Abzum-Vortex/issues/30); final isolation evidence still needs its own exact Testing receipt. [#266](https://github.com/Abzum-NZ/Abzum-Vortex/issues/266)'s Production reconciliation is parked until project end and does not block this Testing path. [#257](https://github.com/Abzum-NZ/Abzum-Vortex/issues/257) remains a non-gating scale follow-up.
...
- Add tenant-administrator assignments, system-only initial provisioning, protected hierarchy/lifecycle commands, safe read models, expected-revision command concurrency, duplicate protection, and typed audit-ready operation evidence in #30. Provisioning composes the explicit Identity-owned runtime-settings initializer from [#430](https://github.com/Abzum-NZ/Abzum-Vortex/issues/430) in its existing transaction; #430 owns settings validation, persistence, updates and the internal runtime reader. Tenant governance uses separate explicit tenant authority; organisation-account and invitation operations require the shared central decision and exact organisation-role permissions in the resolved #27 context. Administrative settings reads use the exact `.read` permission and #430's reader. Neither membership nor tenant-administrator status substitutes for organisation permission. Creates have no prior revision; mutations of existing resources require the expected revision. Generic persisted Activity remains owned by #252/#115.
...
- [Private transactional event append #400](https://github.com/Abzum-NZ/Abzum-Vortex/issues/400) installs the immutable outbox and Basic logged [Supabase Queue](https://supabase.com/docs/guides/queues). [Event delivery #60](https://github.com/Abzum-NZ/Abzum-Vortex/issues/60) then adds webhook wake-up, sequence barriers, retries, failed-event handling and the recovery call.
