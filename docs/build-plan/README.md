...
10. Independent review and normal required checks precede merging to `testing`. A task is marked Done only after the exact current Testing revision is deployed to `vortex-testing.abzum.com` and its required verification succeeds there. Migration changes may be batched with cumulative Testing validation; track and resolve any failures before closing affected tasks. Production deployment is reserved for final project delivery. This policy does not claim that earlier deliveries met the previous per-migration receipt rule.
