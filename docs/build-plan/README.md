...
Current implementation is the explicit person/current-account parameter correction within [#36](issue-36-ownership-and-visibility.md#next-condition-correction-compare-a-person-field-with-the-current-account), separate local-role/template reads within [#40](issue-40-protected-access-administration.md#next-safe-read-checkpoint-local-roles-and-application-role-templates), and the demonstrated [assignment-revocation audit correction #318](https://github.com/Abzum-NZ/Abzum-Vortex/issues/318). The latter preserves real expiry, revisions and the existing stewardship composition; it adds no clock investigation or infrastructure work and does not reopen approved [#33](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33).
