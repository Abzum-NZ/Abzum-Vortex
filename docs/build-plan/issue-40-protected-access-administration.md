...
2. **Useful role configuration.** Summaries contain identity, key, label, role kind, lifecycle, current revision, privilege classification, policy kind and the application/source-role reference where applicable. Detail adds description, current activation-policy settings and the ordered accepted permission configuration. These are accepted configuration facts, not effective access or a promise that the role is assignable. Omit assignment facts, continuity counters, fingerprints and audit internals.
