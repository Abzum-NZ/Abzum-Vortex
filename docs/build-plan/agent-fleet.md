...
7. Shape each selected leaf before dispatch (see [shape before dispatch](agent-coordination.md#shape-before-dispatch)): make the issue clear and bounded, and split it into sub-issues when it exceeds one bounded session. Read current source and bound remaining work for enough eligible leaves to maintain eight active implementer lanes by default, or up to twelve when isolation and capacity are clear. Already implemented functionality goes to a bounded Opus 5.5 or GPT-6 Sol source review and completion reconciliation, not reimplementation. Otherwise select routing/estimate and dispatch the implementer.
