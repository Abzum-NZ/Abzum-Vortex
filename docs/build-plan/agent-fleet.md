...
7. Read current source and shape enough eligible leaves to maintain eight active implementer lanes by default, or up to twelve when isolation and capacity are clear. Shaping makes each issue clear and bounded, and splits it into sub-issues when it is larger than one session (see [shape before dispatch](agent-coordination.md#shape-before-dispatch)). Already implemented functionality goes to a bounded Opus 5.5 or GPT-6 Sol source review and completion reconciliation, not reimplementation. Otherwise select routing/estimate and dispatch the implementer.
