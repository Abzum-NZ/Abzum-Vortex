...
2. Contextual searchable palettes, central editing canvas and right-hand inspector matching the retained prototype. Create an application from blank through the existing services, then edit its composition, navigation, pages and behaviours. Bound Modules use their own draft/publication operations and independent versions; the workspace shows pending Module publication, exact Application binding and deliberate installation upgrade separately. Application-contained row scope does not create Application-owned data definitions. Exact versions are visible during release/install review; mutable labels never determine permanent identity.
