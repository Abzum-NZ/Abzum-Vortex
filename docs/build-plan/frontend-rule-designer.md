...
- The first application uses current-user execution. Later scoped identity, interactive continuation, durable handoff and managed-flow extension scopes remain separately scheduled in the current roadmap.
...
- Platform-managed flows expose permitted parameters and extension slots while hiding private internals. Compile allowed extensions into one exact graph. Installation, copying and upgrades do not copy execution grants or silently broaden authority.
