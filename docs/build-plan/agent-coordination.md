...
Provider concurrency, rate-limit and model-unavailable responses are capacity failures rather than approval denials. Once the exact attempt is positively settled, preserve its work, clear its dead owner, record the capacity result and reassign the leaf to another suitable authorized workhorse without waiting for user redirection. A blocked or failed provider lane must not idle unrelated eligible work in the same phase.
