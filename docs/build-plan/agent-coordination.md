...
The reviewer is a separate session from the implementer, even when both use the same model. It takes exclusive edit ownership of that issue after its implementer stops. One main orchestrator coordinates several independent active leaf issues. The default operating target is six active implementer lanes, expandable to eight when the earliest incomplete phase has enough bounded, dependency-ready work with non-overlapping owning paths. Review lanes are separate: up to four Opus 5 or GPT 5.6 Sol reviewers may work concurrently, and a review or integration wait must not idle independent implementer lanes. Parent and phase epics are rollups, not extra implementation tasks.
...
Provider concurrency, rate-limit and model-unavailable responses are capacity failures rather than approval denials. Once the exact attempt is positively settled, the orchestrator preserves its work, clears its dead owner, records the capacity result and reassigns the implementation to another suitable authorized workhorse without waiting for user direction. Reviewer capacity is unchanged: when no qualified reviewer is available, the leaf remains In review with a capacity blocker rather than moving to a cheaper reviewer. A blocked or failed provider lane must not idle unrelated eligible work in the same phase.
