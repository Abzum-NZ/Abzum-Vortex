...
Provider concurrency, rate-limit and model-unavailable responses are capacity failures rather than approval denials. Once one exact attempt is positively settled, the orchestrator preserves its work, clears its dead owner, records the capacity result and reassigns the implementation to another suitable authorized workhorse without waiting for user direction; do not repeatedly probe or retry the same failed provider or terminal. Reviewer capacity is unchanged: when no qualified reviewer is available, the leaf remains In review with a capacity blocker rather than moving to a cheaper reviewer. A blocked or failed provider lane must not idle unrelated eligible work across the queue.
