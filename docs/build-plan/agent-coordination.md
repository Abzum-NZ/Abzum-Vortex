...
| Reviewer: Claude Opus 5.5 or GPT-6 Sol | Independent review, own findings/fixes, re-review, PR integration, assigned issue update and closure, and its own row's In review / Done transition | Other board rows, dependencies, parents, unrelated issues, sending ordinary fixes back to implementer |
...
4. **Workers report their own transitions.** An implementer sets its own issue row to In progress on a confirmed start and In review on handoff; a reviewer sets its own row to Done, clears Current owner and records the finish time after merge and closure. Each is one targeted write with readback. The orchestrator verifies these, and owns dependencies, parents, pickup and every other row.
...
7. **Stalls are visible.** If the orchestrator has made no progress for 30 minutes while work is pending, the monitor alerts the user with the exact state and the action needed. Workers never wait on a notification that cannot arrive; they follow the stall-recovery table in fleet operations.
