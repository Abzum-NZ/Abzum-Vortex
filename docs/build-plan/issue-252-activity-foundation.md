...
5. Record successful evidence in the same transaction as the owning change. If either fails, both roll back. A protected owner may record a clean permission refusal before its first write and return from that same transaction. Only established local scope may be retained; an unknown or foreign identifier submitted by a caller is not a local subject. A refusal-recording failure never changes the operation to success.
...
- [ ] A refused mutation leaves no completed entry; a clean permission refusal recorded before the first write commits through its owning request without unknown or foreign target content.
