...
The [private delegation composition in #33](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33) stores these changes atomically with one Access-version increment. It does not establish who may request them. The later protected invocation checks management permission and the caller's and approver's permitted before-and-after scope, including onward delegation, through [#34](https://github.com/Abzum-NZ/Abzum-Vortex/issues/34) and [#40](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40). The permanent-steward safeguard must participate before that invocation is exposed. [IAM #267](https://github.com/Abzum-NZ/Abzum-Vortex/issues/267) supplies the ordinary request and approval experience; editable requests alone never grant authority.
...
The initial appointment grants only the minimum platform management permissions. Afterwards, those permissions remain a required minimum within one direct permanent assignment; separately approved additional permissions do not disqualify that steward. Do not combine several incomplete assignments or different accounts to satisfy this safeguard. The [protected administration boundary](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40) still checks all additional authority normally.
