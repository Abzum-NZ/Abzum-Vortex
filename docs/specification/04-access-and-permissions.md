...
The [authored module](../../contracts/src/module-source-contracts.ts), [authored application](../../contracts/src/application-source-contracts.ts) and [compiled action contracts](../../contracts/src/module-contracts.ts) retain their existing single-permission representation. The planned #35 extension adds an explicit, mutually exclusive alternatives list only for multiple permissions; both forms resolve to the same complete-pair decision. Existing published files are not rewritten. An interface's own exposure permission remains an additional boundary, not another record alternative that can supply missing row authority. Flow/action execution remains with its owning engine; this extension defines authorisation metadata only.
