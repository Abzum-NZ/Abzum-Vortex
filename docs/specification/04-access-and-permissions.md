...
The database decision function and declared permission vocabulary define canonical access meaning. The server library preserves that meaning when mapping non-row operations to their protected targets. A TypeScript helper does not replace the PostgreSQL row boundary; both implement the [functional access requirements](20-quality-and-acceptance.md#functional-quality).
...
[implementation plan](../build-plan/issue-34-access-decision.md)
...
All user-facing role grants and access-expanding changes use the [IAM Vortex application](appendices/iam-application.md), including its user-linked request, review and assignment views and generic workflows. Organisation ownership remains unchanged. IAM calls the protected Access operations; editing an ordinary request or review record cannot grant access. Other administration applications link to IAM instead of offering parallel role-grant forms. Its guided first-steward setup and immediate removal paths are defined in the same application contract.
...
Permitted administrators can browse current registered permissions and inspect one exact entry as a read-only [system record type](appendices/core-contract-boundary.md#system-modules) over the protected catalogue, read through the one [query path](10-queries-reports-search.md#one-read-path) under their current authority. It shows the permission's reference, name, description and declared action, not internal publication or audit evidence. The same module permission installed in two applications remains two distinct entries. Withdrawn and historical declarations are excluded; a current declaration awaiting role acceptance remains visible. Visibility in this catalogue never establishes permission to use, assign or delegate that entry.

Permitted administrators can also inspect current local role configuration and separately registered application templates as read-only [system record types](appendices/core-contract-boundary.md#system-modules) over the protected catalogue, read through the same [query path](10-queries-reports-search.md#one-read-path). Local role details show the accepted permission configuration and safe assignment-policy settings, including roles awaiting acceptance or no longer available. A template comes from the exact currently registered application release, not the latest draft. Neither view establishes effective access or grants authority, and the same source role in two applications remains two distinct template references.

Permitted administrators can inspect role assignments and delegation authorities as separate read-only [system record types](appendices/core-contract-boundary.md#system-modules) over the protected live Access facts, read through the same [query path](10-queries-reports-search.md#one-read-path). Each shows its current holder, fixed time window, revision and stored state; delegation details show only the exact permitted scope references, not internal acceptance evidence. Revoked and expired facts remain visible for administration. A displayed active window does not establish effective authority: the current permission decision still applies.
...
[approved cleanup contract](../build-plan/issue-40-protected-access-administration.md).
...
Delegation management has three operations: grant a new delegation, replace an existing delegation's complete scope, or revoke it. Granting names one organisation account or Group and an explicit start and optional expiry. Replacement keeps that holder, identity, original grant evidence and time window; it may explicitly refresh a stale bounded scope or change between bounded and organisation-wide management. The proposed scope must be valid now, but stale old permissions do not prevent their replacement. An unchanged scope is not a change. Expired delegation cannot receive replacement scope; renewal is a new grant with a new identity. Revocation is terminal and remains possible after expiry or source inactivity. Independent grants stay independent.

The [private delegation composition in #33](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33) stores these changes atomically with one Access-version increment. It does not establish who may request them. The later protected invocation checks management permission and the caller's permitted before-and-after scope, including onward delegation, through [#34](https://github.com/Abzum-NZ/Abzum-Vortex/issues/34) and [#40](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40). The permanent-steward safeguard must participate before that invocation is exposed. [IAM #267](https://github.com/Abzum-NZ/Abzum-Vortex/issues/267) supplies the ordinary request and review experience; editable requests alone never grant authority.
...
The initial appointment grants only the minimum platform management permissions. Afterwards, those permissions remain a required minimum within one direct permanent assignment; additional permissions granted separately do not disqualify that steward. Do not combine several incomplete assignments or different accounts to satisfy this safeguard. The [protected administration boundary](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40) still checks all additional authority normally.
...
A record update must pass the complete access check for both its current and proposed state. If authority expires between those checks, the later check refuses the update; an earlier successful check cannot extend access. The [row-enforcement implementation](../build-plan/issue-35-row-policy-composition.md) keeps revisions and held Access facts authoritative without adding a second timing or permission-cache mechanism.

The record-scoped permission explicitly declares its supported visibility routes. Scope is part of that permission's published meaning and current catalogue evidence, not inferred from a role label, a record's ownership mode, or a client-supplied filter. A record can have an owner while an authorised permission allows wider visibility; a record with no owner is not automatically visible. Missing required scope refuses. Every current record permission declares its scope; absent scope cannot provide record authority. [The #36 implementation plan](../build-plan/issue-36-ownership-and-visibility.md) owns complete authored-source, compiler, meaning and database mapping without rewriting immutable releases.
...
[adapter handoff](../build-plan/issue-35-row-policy-composition.md)
keeps these responsibilities separate without another permission system.

Comparing a person field with the current user requires an explicit organisation-account-reference parameter. Its value comes from the verified current organisation account, never a user-entered identity. An ordinary text parameter does not silently become a person reference. The [typed-parameter implementation checkpoint](../build-plan/issue-36-ownership-and-visibility.md) adds this missing declaration while preserving historical text semantics and publication evidence.

Conditions validate calendar dates and require offsets on date-times. Date-times compare integer microseconds with up to six fractional digits, including pre-epoch instants and year zero within the declared offset range; millisecond-only parsing must not truncate them. Text ordering is Unicode code-point order. JSON equality ignores object-key order and preserves array order. Explicitly declared finite-number operands use the same finite-double comparison domain in JavaScript and PostgreSQL; this does not claim arbitrary-precision numeric behavior. Declared decimal and money operands instead use the current exact-value rules: amounts remain exact, money equality includes currency, and ordering across currencies is refused. A permissive database cast cannot expand the accepted input language.
...
Changing any fingerprinted value withdraws the earlier decisions and requires both sides to review the new proposal. The Access service owns the grant and activates it only after verifying both immutable consent decisions. An ordinary editable record has no security effect on a grant.
...
- All access entry points use the same declared permissions, organisation scope and protected operation boundaries.
...
- Directly changing an ordinary record cannot activate a sharing grant.
