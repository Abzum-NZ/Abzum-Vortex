...
53. Use the same organisation role-management operations for direct-account and Team assignments to organisation-wide and application-specific roles. Prove another organisation's account, Team, role or application cannot be substituted, and removing an assignment changes the next access decision.
...
55. Concurrently demote, suspend or revoke the final permanent organisation steward, including through a role edit or cluster identity-projection change. At least one effective direct permanent steward must remain. An expiring or Team-only delegate cannot be used as the replacement; pre-existing organisations require explicit adoption and no first-account inference.
56. Run direct and indirect role-grant journeys through the [IAM application](appendices/iam-application.md): requests and approvals remain linked to the exact organisation accounts, Teams, role versions and application contexts; no request has effect until the protected operation succeeds. Cover Team membership, role expansion, template acceptance/reactivation, invitations and delegation, not only direct assignment.
