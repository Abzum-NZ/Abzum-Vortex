...
Include positive retained-access proof, not only refusal cases: metadata changes, narrowing and pending additions preserve still-approved activation use, with removed permissions ineffective immediately. Accepted added/restored authority requires fresh activation without cancelling eligibility. Compatible eligibility survives same-mode policy-detail changes, while old requests/windows fail and fresh activation obeys the new exact policy. Every standing/activation-required mode switch and return checks the complete non-revoked assignment manifest, including scheduled and naturally expired rows; no kind is converted, missing/extra/stale entries refuse, and revoked/expired grants remain ineffective. The same cases cover direct and Group assignments, current source revisions and two-organisation isolation. Use the [retained-authority rules](appendices/groups-and-privileged-access.md#retained-permissions-during-role-review), not equality to the latest role revision or duplicated activation permission lists.
