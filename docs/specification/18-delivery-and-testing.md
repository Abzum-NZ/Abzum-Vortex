...
The shared Kestra instance uses distinct environment namespaces, flow identities, webhook authentication, Doppler configurations and least-privilege target credentials, and retains the separate Testing evidence and Production approval gates below. This is logical and credential separation inside one trusted operated service, not strong process isolation. It neither assumes that a new Development credential has been provisioned nor permits one environment's flow to read another environment's database, file store, queue, workflow state, cache, or secrets.

Authorised operators may restart or redeploy the shared Kestra instance when needed for service operation, after [issue #271](https://github.com/Abzum-NZ/Abzum-Vortex/issues/271) has established a fresh, consistent and recoverable shared-service state covering both its state database and paired internal `kestra-storage`. They may upgrade its pinned version only through the independently tested compatibility and state-migration work in [issue #198](https://github.com/Abzum-NZ/Abzum-Vortex/issues/198), including its backup, disposable restore rehearsal and forward-migration controls. That operating authority does not approve a Production database connection, migration, reset or release: the Production flow must still stop at its named approval gate and satisfy every ancestry, migration-set and evidence check before separately authorised delivery.
