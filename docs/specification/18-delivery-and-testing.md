...
Authorised operators may restart or redeploy the shared Kestra instance when needed for service operation. A future upgrade of its pinned version remains separately governed by the independently tested compatibility and state-migration work in [issue #198](https://github.com/Abzum-NZ/Abzum-Vortex/issues/198). That operating authority does not approve a Production database connection, migration, reset or release: the Production flow must still stop at its named approval gate and satisfy every ancestry, migration-set and evidence check before separately authorised delivery.
