...
**Engines first.** Before building the App Designer, prove that a complete application can be defined in files, validated, published, installed and used in the normal browser runtime without a visual editor. The application renderer, record/query services, access decisions and configured flow engines are prerequisites, not designer features. The [engine-first delivery proof](../build-plan/engine-first-application-delivery.md) includes real data, forms, actions, packaged workflows and the existing cross-application fixture scenarios. A static prototype is not that proof. The later designer and MCP authoring tools change the same definitions through the same protected operations; they do not introduce another renderer, save path or workflow engine. Further prototype expansion is deferred until the runtime proof is complete.
...
The Landing Zone is an ordinary application that gives each person a personal start page inside one organisation. Each person's tiles are records owned by their organisation account, in application-contained storage, under permissions whose only record scope is ownership; no permission in the application reads another person's tiles. A tile names an application, a page, a record or an external address and carries an open behaviour, a favourite flag and order, and the time the person last opened it through the Landing Zone. Recently used shows only targets opened through the Landing Zone; the platform records no other page visits.

The page uses one shell with a required main slot and an optional rail slot. The rail holds registered blocks chosen in the application release and renders empty when none is placed; a block whose capability is unavailable in the organisation renders its unavailable state. The platform contributes only the permitted-applications read, the organisation default application and generic link, launcher and filter blocks; favourites, recency and bookmarks are definition semantics.
