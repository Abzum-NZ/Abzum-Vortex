...
Initial provisioning explicitly nominates an organisation account. The existing Identity provision/adopt operation establishes that nomination and management authority; it does not grant entry to business applications. Initial application installation and operating rights require a separately scoped setup capability and the protected composition owned by [#72](https://github.com/Abzum-NZ/Abzum-Vortex/issues/72). Subsequent access expansion follows the ordinary [IAM request and approval journey](appendices/iam-application.md).

Before the first setup mutation, bind the original provisioning receipt and expected setup revision to one server-owned manifest containing the exact organisation, nominated account, initial application releases and operating roles with their permission scope. Freeze that manifest identity and fingerprint for the setup attempt. A retry must present the same receipt, revision and manifest and return or resume the recorded outcome; changing a release, beneficiary or permission set is not a retry. Setup cannot become a later general grant path, infer a beneficiary from first sign-in, or reuse tenant-administration status as application authority.

App coordinates installation through the existing protected Module lifecycle and invokes Access's narrow initial-operating-rights operation. Access validates the trusted nomination, frozen scope, current role/release facts and expected revisions, composes its private role/assignment and management-requirement writers with Activity, and preserves final-steward safeguards. Access imports neither App nor Page; it returns a typed setup outcome to App. An incomplete attempt remains resumable against its fixed manifest and does not advertise usable access before the relevant installation and rights are effective. Browser callers cannot supply trusted setup context, and ordinary runtime requests always use current session and Access decisions.

A development setup command may select the fixed shipped development manifest and call this same protected composition under its explicit setup capability. It cannot call raw owner helpers, issue arbitrary grants, use runtime fixture roles, or bypass normal record permissions. Completion records the selected releases and effective operating rights; it does not copy general tenant operator authority into an application.
...
The current representation defines omitted view/use restrictions explicitly. Permission changes participate
...
The only hand-written screens are the bootstrap screens: sign-in, registration, recovery and confirmation, session ended, the organisation chooser, error pages, and the designer workspace. They are platform screens rather than application definitions, so they cannot be selected by a tenant short name. Every other screen — including Tenant Administration, Organisation Administration and IAM — is an ordinary published application page reached through its own application address. A bootstrap screen still ships no meaningful operation that exists only as handwritten click behaviour: each operation it offers runs through a registered flow binding and its protected owning operation exactly like an application page (see the [semantic interface map](#semantic-interface-map)).
