...
Support access is a protected support grant created by a read-only Access operation, delivered in Phase 13 using Identity, Access and Activity. The security operator and support operator are ordinary role templates, assignable to any user in the organisation through the ordinary catalogue; they are not a special actor class, and the operator cannot grant their own support access. The operator signs in under their own global identity with current strong/recent authentication and an active support-operator role. The grant names the exact operator, existing subject organisation account, organisation/application, permitted view/scope, purpose, ticket and expiry. The protected operation checks only current authority and its declared preconditions (reason, maximum duration and recent authentication); it has no approval field and performs no approver check. An organisation that wants approval builds a Kestra flow with a wait-for-a-person task that then calls that operation; if approval must be unavoidable, the operation's policy names the published flow that is its only permitted caller. Ticket creation or editing and operator-role eligibility alone never grant access.
