...
- The first release uses only five closed calculation forms: join named text fields, apply one of four numeric operations over named field/literal operands and further numeric operations, evaluate a typed condition, offset a named date/date-time field by a named/literal amount, or determine whether a named deadline has passed while excluding explicitly listed terminal status values. Each form is a formula over that catalogue, and a nested numeric operation is the same closed numeric form, so a new formula needs no new expression and no engine change. Numeric operands nest to a bounded depth; money keeps its currency, and mixed currencies, an undefined money dimension, dividing by zero, and a result that overflows the declared result type are refused. The deadline-passed form is the only read-time form; it is never stored, a calculation that uses it is itself read-time, and publication refuses a total whose aggregate source or aggregate-source filter depends on it. The declared result type must match that form. Arbitrary objects, scripts, and user-defined expressions are refused.
