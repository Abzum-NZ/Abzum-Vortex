...
Intermediate Continue/Submit validates answers into the private journey draft; only final Submit invokes a configured bounded owning operation when the journey has one. Several required forms must all be satisfied before any business changes commit. Cancel or abandonment before submission commits no business effects, event, activity effect or workflow-start intent. The server recomputes required gates and validates answers, access and expected revisions rather than trusting a client-supplied next node. No transaction or server request remains open during human input. Existing draft storage and revisions provide private persistence and safe resume; no separate durable frontend-run store is introduced.
