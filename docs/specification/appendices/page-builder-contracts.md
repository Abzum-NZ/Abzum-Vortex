...
Vortex is a new application. Use the complete shell, slot, nested placement, guided-step, typed setting and responsive layout contracts as the sole application representation. Remove obsolete Application V1 readers, aliases, default selectors and conversion operations under [#249](https://github.com/Abzum-NZ/Abzum-Vortex/issues/249). Contracts, compiler, Definition storage/read/restore, Page projection, installation provisioner and Puck adapter change together.

The current draft uses revision-checked Definition save. Shell and slot aliases resolve to permanent source identities. Duplicate identities, stale revisions and invalid mappings refuse the whole save. Saving does not publish or select an installed release.

Publication stores the exact authored source, canonical content and declared current contract metadata using the existing Definition store. The source and validation metadata must agree with the decoder; unknown formats are refused rather than guessed. Release identity, dependency manifests, canonical fingerprints and explicit installation selection remain product behavior. They do not require a second platform format or an old-reader compatibility service.

History and restore operate on the current representation. Restore creates an editable draft; it does not change the active release. Correct development data and checked-in definitions at the same owning boundary when the platform schema changes. Applied database corrections use a forward migration; this instruction does not reset shared environments.
