...
This section describes target runtime behaviour. Current-person execution uses the initiating person's protected request context. [#322](https://github.com/Abzum-NZ/Abzum-Vortex/issues/322) later implements Access-owned binding, per-node resolution and viewer-safe handoff for specified and System actors. Actual row/field/derived/file/component/MCP integration remains in the downstream owning tasks; it does not make the identity foundation depend on the later flow engine.
...
Current rules use one explicit graph representation; obsolete single-effect readers and conversion paths are removed. See the
...
| All node schemas and dependency references; graph order/cycle refusal; variable defaults/types/branch assignment; no cross-run leakage; one current graph representation                                                      | [#58](https://github.com/Abzum-NZ/Abzum-Vortex/issues/58)                                                                                                                                                                                    |
