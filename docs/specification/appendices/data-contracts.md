...
A **rule** is a flow with a `BeforeSave` trigger and the `transaction` execution kind. It declares identifier, owner/subject context, an explicit contract version, typed inputs and variables, an optional entry condition, a priority, registered versioned tasks and declared terminal outcomes. The [Frontend Rule Designer contract](frontend-rule-designer.md#one-flow-language) governs the complete versioned extension and fixtures. Rules use the one flow representation throughout authoring and execution; the obsolete rule-graph reader and conversion path are removed. Module rules retain their record-only context; application rules can additionally refer to owned pages, forms and flows. There is no separate permission-approval rule.

The first implementation profile is the `BeforeSave` flow, embedded in the
explicit Module source/validation pair `3.0.0`. It uses the current exact-value
field values and registers the control tasks **If**, **Switch**, **Stop** and
**Sequential** with the pure tasks **Set variable**, **Set field**, **Require field**,
**Warn** and **Refuse save**. Typed run-local variables are not infrastructure
