...
A group has `group_id`, organisation, key, label, state, creator, and creation/change times. A membership has organisation, group, organisation account, state, start, optional expiry/revocation, revision and trusted operation evidence. The pair of group and organisation account is unique while active; one account may have several memberships. Cross-organisation membership is invalid. Actual same-organisation and uniqueness constraints belong to [protected storage #33](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33); a structurally valid identifier alone does not prove membership. A future Activity record may link to the operation evidence but is not required by the initial private contracts.
