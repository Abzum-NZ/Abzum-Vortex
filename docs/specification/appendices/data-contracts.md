...
The exported result is a strict kind-discriminated safe projection containing only `kind`, `organizationId`, `definitionKey`, `rootId`, `releaseRevision`, stable `releaseVersion`, `validationContractVersion`, `contentFingerprint`, `resolutionFingerprint`, complete canonical `content`, sorted complete `dependencyManifest`, and `correlationId`. The manifest uses one exact dependency entry per Module, connection type or platform theme, in deterministic subject order.
