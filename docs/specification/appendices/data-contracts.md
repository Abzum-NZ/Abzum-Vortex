...
An immutable activation-policy revision belongs to one exact organisation and role. It contains its permanent identity, revision and fingerprint; `maximumActivationDurationSeconds`; `reasonRequired`; `recentAuthentication`; `independentApprovalRequired`; and trusted change actor, time and correlation. Recent authentication is explicitly `none`, or `primary`/`multi_factor` with `maximumAgeSeconds`. Durations, ages and revisions are positive safe integers, and timestamps must be finite. A role cannot reference another organisation's or another role's policy. The [policy continuity rules](groups-and-privileged-access.md#privileged-classification-and-policy) prevent returning to an older policy from reviving stale activation evidence.
