...
Current collective-principal contracts use Group identities and the `group` discriminator. Authored and compiled Definition ownership values serialise `group` and the stored Access-version reason is `group_membership_changed`; the legacy `team` and `team_membership_changed` spellings and their converters no longer exist, so no current reader or writer accepts them. [#283](https://github.com/Abzum-NZ/Abzum-Vortex/issues/283) reconciles the current contracts before assignment delivery. A terminology change does not implement PIM or authorise rewriting published artifacts, permanent permission keys or applied migrations; an already-published release whose record type carries ownership `team` is republished from a corrected definition rather than migrated in place.
