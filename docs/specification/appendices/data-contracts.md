...
An active identity-session result has exactly `identity_id`, Supabase `session_id`, `authentication_strength`, `access_token_issued_at`, and `access_token_expires_at`. Expiry must still be in the future when the result is created. The closed resolution kinds are `active`, `missing`, `expired_or_revoked`, `cluster_identity_inactive`, `temporarily_unavailable`, and `invalid_session_state`. A temporary Auth/JWKS/network or cluster-projection-store failure maps to the same safe public result; internal telemetry may retain the non-secret operational cause. No variant contains an access token, refresh token, email, provider user/profile, tenant, organisation, organisation account, Access version, application, role, permission, grant, capability or business value.
