...
A group has `team_id`, organisation, key, label, state, creator, and creation/change times. A membership has organisation, group, organisation account, state, start, optional expiry, grantor, and activity reference. The pair of group and organisation account is unique while active; one account may have several memberships. Cross-organisation membership is invalid.
...
Historical serialized `team` references below identify the same collective principal now called a Group. [Compatibility #283](https://github.com/Abzum-NZ/Abzum-Vortex/issues/283) must reconcile current Group contracts and all readers/writers explicitly before assignment delivery. These documentation changes do not claim that renamed contracts or PIM are implemented, and do not authorise rewriting published artifacts, permanent permission keys or applied migrations.
