...
flowchart TD
    ...
    EVENT --> RUN[Flow runs and callbacks]
...
Each customer-managed Definition root has one permanent source identity whose identifier is the root identifier. Every contained source component has one database-allocated permanent identifier, definition root, component kind, authored owner `id`, stable owner scope, creation time, and creating actor. The stable owner scope uses the owning component's `id` for nested fields, relationships, guided steps, flow tasks, and interface operations; it never depends on a mutable parent key.
...
Ordinary identity tokens use no custom access-token hook because the standard claims are sufficient. Confirmation and recovery templates use the provider-supported `ConfirmationURL`. After Supabase verifies the single-use email token, it redirects to an exact allowlisted Vortex callback with an implicit-flow session fragment. Browser code removes the complete fragment from the address and history before validating its closed purpose and sending request-local credentials in a same-origin server-action body. Confirmation accepts `type=signup` and sends only the access token; the server validates it with `getUser`. Recovery accepts `type=recovery`, sends its access/refresh pair with the new password, and uses a non-persisting Supabase client to set that request-local session and update the password. Neither operation stores a token, enables automatic refresh, sets a durable Vortex cookie, or exposes a provider error. Organisation accounts consume the verified identity identifier and email under their own lifecycle rules. Session handling consumes the same verified result and separately owns `@supabase/ssr`, cookie persistence, refresh, sign-out, revocation, and global-identity state checks.

### Identity session

An active identity-session result has `identity_id`, Supabase `session_id`, `authentication_strength`, `access_token_issued_at`, and `access_token_expires_at`. The [recent-authentication extension](recent-authentication.md#minimum-evidence-and-ownership), owned by #276, adds only optional verified primary and multi-factor confirmation times to this closed result and the verified Identity result that supplies it. Expiry must still be in the future when the result is created. The closed resolution kinds are `active`, `missing`, `expired_or_revoked`, `cluster_identity_inactive`, `temporarily_unavailable`, and `invalid_session_state`. A temporary Auth/JWKS/network or cluster-projection-store failure maps to the same safe public result; internal telemetry may retain the non-secret operational cause. No variant contains an access token, refresh token, email, provider user/profile, tenant, organisation, organisation account, Access version, application, role, permission, grant, capability or business value.

Supabase Auth owns the durable session. Vortex stores no session row or token copy. The provider credential pair exists only inside one request-specific server client and a server-managed cookie family. Hosted environments use host-only `__Host-vortex-session` chunks with `Secure`, `HttpOnly`, `SameSite=Lax`, `Path=/`, high priority and no `Domain`. Exact HTTP loopback uses non-prefixed `vortex-local-session` chunks with the same attributes except `Secure`. Exactly one unchunked base value or one gap-free decimal `.0` to `.N` sequence is accepted, using 3,180-byte chunks, at most eight chunks and a 25,440-byte aggregate limit. Mixed generations, gaps, non-decimal suffixes and oversized state are invalid.

The browser has no Supabase Auth client. Proxy constructs a request-specific SSR client and uses `getClaims()` only for supported refresh and optimistic token checking. Proxy overwrites an internal request-state marker; a protected resolver does not retry provider refresh after Proxy reports a temporary failure. After a verified Proxy result, protected server operations independently verify the current access token through the Identity verifier and read the cluster projection through a non-mutating runtime-only operation. Supabase owns refresh-token lineage, rotation, reuse handling and parallel-refresh convergence. Vortex verifies every resulting current or refreshed access token and accepts only a complete canonical cookie mutation set from the official client; it does not claim access to pre-refresh evidence that client no longer exposes or control over browser response ordering. Provider credentials remain bearer credentials and are not device-bound.
...
A group has `group_id`, organisation, key, label, state, creator, and creation/change times. A membership has organisation, group, organisation account, state, start, optional expiry/revocation, revision and trusted operation evidence. At most one non-revoked membership exists for an organisation/group/account pair, including scheduled and naturally expired rows; one account may belong to several groups. Renewal closes the previous row before creating a new membership identity. Cross-organisation membership is invalid. Actual same-organisation and uniqueness constraints belong to [protected storage #33](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33); a structurally valid identifier alone does not prove membership. A future Activity record may link to the operation evidence but is not required by the initial private contracts.

The Group's organisation, identity and key remain permanent; its display label may change. Retiring a Group does not release its key for a different Group or erase the source identities needed to explain earlier assignments. An organisation may add its own extension fields to the Group projection; their values are ordinary organisation data keyed to the Group and never part of the protected Group contract.
...
Only the human context gains the optional verified confirmation times from [#276's evidence contract](recent-authentication.md). When evidence is present, it also carries the verified access-token issue time as an upper bound, distinct from context creation. Ordinary contexts with no confirmation evidence retain their current shape. Browser input and authentication-strength labels cannot supply these times; no corresponding extension is inferred for federated, system or public callers.
...
The human organisation resolver, context initialization, `SET LOCAL ROLE`, live context validation and protected operation execute in one database transaction. The resolver keeps shared locks on the selected Identity and Access scope rows until that transaction ends. The database preserves the complete union as one owner-only row bound to the establishing transaction; it cannot be replaced, re-established or cleared by the request role or the runtime login within that transaction, and no session setting carries it. Structural validation and setting that value do not grant authority. The trusted server begins the transaction as `vortex_runtime`, executes the initializer available only to that role, and then enters `vortex_request` with `SET LOCAL ROLE`; only the request role may call the read-only context accessors and Access-owned live-context validator used by protected service SQL. The request role has no Identity schema access. Commit, rollback or pooled connection reuse cannot carry the role or value into another transaction.

A flow never mutates this initiating human context. A future task execution binding may select `current_actor`, one exact specified organisation account, or one registered system actor, but specified/system execution also requires the separate exact [scoped execution-identity authority](https://github.com/Abzum-NZ/Abzum-Vortex/issues/322) for the published flow and task. Every identity-changing protected task starts a new Access-resolved transaction and establishes the effective actor's complete closed context; no actor UUID, browser value, flow variable or service-role credential can replace that resolution. Results returned to a different initiator use the initiator's current viewer-safe projection, and subsequent writes re-authorise their own inputs.
...
The [IAM application](iam-application.md) owns ordinary access-request and review records and their flows. Core retains only the existing protected effective role, assignment and delegation facts and generic operation/human-response evidence required to secure them. IAM links proposals, responses and operation outcomes to exact organisation accounts or Groups; it must not create an independently writable assignment copy. Editing a display record cannot supply approval authority. Full grant orchestration is [#267](https://github.com/Abzum-NZ/Abzum-Vortex/issues/267), not a new core IAM record schema or approval task type.

The [organisation-managed catalogue](../04-access-and-permissions.md#one-organisation-managed-catalogue) is the ownership boundary for all live roles and assignments. Published application roles are immutable templates, not global assignments. Registration records the organisation, exact local application root and release, source role identity, permission-catalogue fingerprint and accepted exact permission identities. The organisation, owner definition and permanent permission identity determine scope; labels and bare keys alone never identify authority across applications.

Organisation-owned roles may select exact permissions from several registered applications or modules in that organisation. A custom role made from an application template is independent live organisation data and is not overwritten by later application releases. Registration creates no account or Group assignment. Existing assignments do not receive newly added or broadened application permissions without an explicit authorised review; removed permissions are unavailable. The owning application change and its Access-version increment must be atomic.
...
Every immutable role revision stores the positive safe-integer `authorityContinuityRevision`. Initial revision is one. An immediate successor preserves this number exactly when its sealed authority tuples are a subset of its predecessor's and it is not a transition from `unavailable`/`retired` to `active`/`acceptance_required`; all other transitions advance it exactly once. A tuple comprises exact application context, owner kind/identity, permission identity, permission continuity and meaning fingerprint, not entry ordinal or acceptance/source provenance. Simultaneous removal and addition advances the number; lifecycle restoration advances it even for equal or empty sets. Policy-only changes use the separate policy continuity rule. Validate the complete set once per revision against the indexed immediate predecessor, with no per-request historical scan or duplicate authority hash.

Period validation is not permission to perform a lifecycle transition. Protected role operations refuse restoration of a retired role; automatic coordination preserves it unchanged. An unavailable role becoming empty and pending, followed by explicit acceptance of added entries, advances the number at each distinct transition.

Activation facts pin the role identity, historical role revision as provenance, authority continuity, policy continuity and exact policy reference, plus exact eligibility and any originating membership identity/revision. The central decision compares the current authority/policy periods and reads current retained entries, not equality with the latest role revision. Authority-period changes invalidate old activation windows, not the assignment itself. Existing eligibility can request activation of the retained nonempty accepted remainder during pending role review; new assignments and pending additions remain disallowed. See [retained PIM permissions](groups-and-privileged-access.md#retained-permissions-during-role-review).

The closed activation fact identifies one `roleActivationId`, organisation account, organisation and role, with its own revision and trusted activation/change/revocation evidence. Its eligibility source is explicitly direct or Group-derived: both require the exact assignment identity/revision; only the Group-derived form requires the exact originating membership identity/revision. Activation begins at the protected operation's successful `activatedAt`, not at a scheduled future start, and requires a finite `expiresAt` later than that time. A temporal read can report active, expired or revoked, but cannot decide permission. It contains no copied permission set, request/approval record, reason, authentication payload or Access version. Current source identity, organisation, account, kind, policy and time bounds are validated by later protected operations and the [central decision](https://github.com/Abzum-NZ/Abzum-Vortex/issues/34), not inferred from successful contract parsing.

Adding `authorityContinuityRevision` to already stored role history is an additive migration: validate complete immediate-predecessor histories and sealed entries, derive the initial value and each transition deterministically, and update only the new column. Refuse inconsistent history rather than infer missing authority. Any narrowly scoped suspension of the immutable-update guard for backfill is transactional and restored before final validation; never rewrite prior accepted permissions, provenance or an applied migration.

An immutable activation-policy revision belongs to one exact organisation and role. It contains its permanent identity, revision and fingerprint; `maximumActivationDurationSeconds`; `reasonRequired`; `recentAuthentication`; an optional `requiredCallerExecutionBindingId`; and trusted change actor, time and correlation. It has no approval field. Recent authentication is explicitly `none`, or `primary`/`multi_factor` with `maximumAgeSeconds`. A named required caller is a current, active flow execution binding in the same organisation when the policy is created; activation then refuses unless it runs under exactly that binding, still current, active and unexpired, with the binding's run-as actor as the actor making the change. Durations, ages and revisions are positive safe integers, and timestamps must be finite. A role cannot reference another organisation's or another role's policy. The [policy continuity rules](groups-and-privileged-access.md#privileged-classification-and-policy) prevent returning to an older policy from reviving stale activation evidence.

Application registration makes templates available but creates no assignable role or assignment. A separate explicit current-template acceptance creates the local role. Its `acceptance_required` lifecycle retains only stored, continuously valid accepted entries for existing non-revoked assignments; candidate additions are not live entries. Only `active` roles accept new assignments. `unavailable` and `retired` contribute no permission. [#34](https://github.com/Abzum-NZ/Abzum-Vortex/issues/34) checks entry continuity for both `active` and `acceptance_required` roles; [#33](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33) checks the complete affected-assignment manifest before accepting a broader or restored revision. Automatic narrowing preserves original acceptance provenance rather than claiming a fresh approval.

The private withdrawal command binds the exact organisation, application and current registration revision. It uses locked current registration facts and marks permission/template availability and supplied roles unavailable without needing a new Definition read. Custom roles remain unchanged and their broken-continuity entries become ineffective. Preparation of a registration, update, reactivation or later template acceptance still requires the exact immutable release evidence.

The coordinated writer in [#33](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33) owns the complete accepted permission-entry set for each role revision: it writes that set atomically and cannot append permissions to a previously accepted revision. It processes every registration transition affecting continuity, including withdrawal followed by reactivation; observing only the final matching state cannot preserve the old authority period. Private storage constraints alone do not complete this writer or make roles assignable to users.

The coordinated application-access command is closed: `register`, `update` and `reactivate` carry verified exact prepared application-role templates with a registration-candidate basis and the nested permission-registration evidence; `withdraw` carries only the exact organisation/application and expected registration revision. Actor and correlation evidence are required. Update and reactivation also require the expected current registration revision. The prepared data is not authorisation: the protected caller establishes authority before invoking the private composition. The raw permission-registry mutation is internal, not a second shipping change path.

The initial composition is owner-only database infrastructure, not a callable runtime/request adapter. The shipping database connection does not use owner credentials, and neither runtime nor request roles may execute this private composition directly. Any owner-handoff adapter used to test command and result binding remains test support and is not exported by the Access service. [Protected operations #40](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40) must supply the concrete authority-checking database entry point before [application installation #64](https://github.com/Abzum-NZ/Abzum-Vortex/issues/64) can invoke it through the real runtime connection. That entry point checks trusted request context, the central decision and applicable delegated scope, then calls the private composition within the protected transaction. Its runtime adapter targets that protected entry point, not the owner-only helper. Existing private catalogue initialisation, metadata-change and read adapters likewise require an explicit protected invocation path before user-facing integration; passing a transaction-shaped object never establishes authority. Do not add an owner connection or broaden raw helper privileges to make an early adapter callable.

The result distinguishes `changed` from `unchanged` and carries operation, organisation, application, registration state/revision, Access version and correlation. Only a current-revision `update` may return `unchanged`: exact registration/history, complete permission and template continuity including retained unavailable markers, and derived supplied-role state must already agree. That result writes no history and increments nothing. A retry with the former expected revision is stale/refused, not a fabricated successful replay. Incomplete or inconsistent continuity cannot be repaired by declaring a no-op. The prior raw registry result remains a separate internal contract.

Complete role-entry sets use the final immutable role revision as their seal. The private writer inserts entries before that revision through a deferred reference in the same transaction. Both entry insertion and final-revision insertion require and lock the same permanent role. Entry insertion refuses if the target revision already exists, preventing late or concurrent additions; a competing final seal cannot assemble another transaction's permission set. The final revision validates the whole set once; entry updates/deletes remain forbidden. All owning operations take the organisation governance lock before role locks. This needs no separate seal record, receipt framework or mutable approval flag, and the accepted-grant fingerprint remains acceptance provenance rather than a repurposed storage checksum.

First observation of an older raw registration is permitted only when both application continuity sets, all supplied-role identities/revisions for the application including empty historical snapshots, and all related current/historical permission entries including custom-role entries are absent. Its initial continuity number is one; its last-processed registration revision identifies the actual locked baseline and need not be one. Partial state or prior accepted authority refuses initialization. Every later transition processes all rows, including unavailable markers, before advancing that baseline; matching final bytes cannot hide an intervening withdrawal. This compatibility boundary creates no user authority and is not a general repair operation.

That safe first observation is a real `changed` update even when the prepared published candidate matches the existing raw registration: it creates the next registration revision, establishes complete tracking and increments Access once. It needs neither a dummy release nor withdrawal. A subsequent exact update at the new current revision is `unchanged`; a retry using the old expected revision is stale. A fully complete no-op can succeed at the maximum registration revision because it increments nothing; a first observation or other actual change at that limit refuses exhaustion.

For already tracked applications, completeness includes continuity identity rows for every current or historical accepted application-context permission and supplied-role source. Missing old unavailable markers refuse rather than being reinitialised at one when an identifier reappears. A withdrawn registration has no available permission or template continuity rows. Both unchanged and changed paths apply these same scoped checks; no general repair or additional history registry is introduced.

The local description is supplied at acceptance, not inferred from an immutable template that has no description. Each exact accepted permission entry records its owning kind/identifier, permission identifier, application context where applicable, accepted registration revision, authority-bearing meaning fingerprint and continuity revision. Platform permissions have no application context; application and bound-module permissions require it. The accepted registration revision remains immutable acceptance provenance, not a requirement to equal the latest registration revision. A continuity revision identifies one uninterrupted period of availability with the same meaning in that exact scope. The private current/tombstone record retains the last processed registration revision and unavailable state: removal, withdrawal, reactivation or changed meaning breaks continuity, including a later return to the original meaning. Display-only changes preserve it. Application role entries, custom role entries and bounded delegation must match current continuity and meaning as well as current catalogue availability; historical matching identifiers cannot revive authority.

An expanded or restored role acceptance binds an exact role candidate and a deterministic affected-assignment manifest: role identity, every non-revoked account/Group assignment identity and expected revision, assignee kind/identity, and manifest fingerprint. The protected transaction verifies the complete current set and current authority; stale, omitted or additional assignments refuse the operation. One accepted role revision governs those assignments without duplicating permission snapshots per assignment. Revoked assignments remain revoked. Bounded delegation is separately accepted. Removal of a supplied template makes its local registration unavailable, and its reappearance requires fresh acceptance even with the same source role identifier. These facts are owned by [#33](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33); [#34](https://github.com/Abzum-NZ/Abzum-Vortex/issues/34) remains the only permission decision and [#40](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40) owns protected changes.

The closed role-change command distinguishes creating a custom role, copying the exact current template into an independent custom role, explicitly replacing custom-role permissions, accepting a supplied application role from the exact current template, editing existing local details/policy without permission acceptance, and retirement. Existing-role changes require the exact expected live revision. Metadata/policy edits preserve the sealed permission set and original source evidence; application lifecycle also stays unchanged. Retirement preserves that snapshot and needs no current Definition or permission-availability lookup. Only explicit permission acceptance/refresh rechecks current registration, template and exact permission continuity before replacing the accepted set. See [editing versus acceptance](groups-and-privileged-access.md#editing-a-role-versus-accepting-permissions).

Canonical role-candidate evidence binds the operation, organisation, permanent role identity, expected revision where applicable, proposed local fields, exact source/provenance, permission entries and policy choice. It excludes the assignment manifest to avoid circular fingerprints. The manifest binds this candidate fingerprint and the deterministically ordered exact assignment identities, revisions and assignees. An explicitly empty manifest is required when review is necessary and there are no non-revoked assignments. These fingerprints use the existing canonical JSON rules and are evidence, not caller authority; the database still validates exact relational facts. Original application-acceptance fingerprints remain separate provenance and are preserved by later automatic narrowing or metadata-only edits.

The proposed activation-policy choice is closed: standing with no policy; an exact existing immutable policy belonging to this organisation and role; or a complete new policy revision. A fresh policy identity begins at revision one; another revision of that identity is its immediate successor. Choosing an earlier exact same-role policy reference is an explicit policy change, so the separate continuity number prevents old activation windows from returning. New policy, sealed role entries/revision, live pointer and one `role_catalogue_changed` Access increment commit together. A refused, stale, duplicate, exhausted or same-state request leaves no partial role or orphan policy. No new receipt/history framework or raw runtime grant is introduced.

The result must match the exact submitted operation, organisation, role, resulting revision, proposed configuration and trusted change actor/correlation. It reports a newly created activation policy exactly when the command requested one, including that policy's complete settings and exact reference; reusing a policy or retiring a role cannot report policy creation. A standalone result shape is not proof of this command/result match. The private hand-off verifies the pairing, while database tests prove that fields the command intentionally leaves unchanged retain their stored values.

The protected server verifies the prepared role/template evidence with the existing canonical preparation verifier before calling the database; browser-submitted evidence is never forwarded unchecked. The database treats those fingerprints as provenance and rechecks exact current registration, permissions, template continuity and affected assignments. It does not introduce a second JSON serialization or hashing implementation. The later [protected invocation task](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40) must preserve this boundary; the current test-only hand-off does not expose a runtime granting surface.
...
A role assignment has organisation, the permanent identity of one live role, assignee kind (`organisation_account` or `group`), assignee identifier, start and optional expiry/revocation facts, revision and trusted grant evidence. Its application scope is derived from the referenced role, not accepted as an independently optional caller field. A direct person assignment names an organisation account, never a global identity. A later Activity record may link to the operation evidence; future Activity persistence is not a prerequisite for initial private assignment storage.

An assignment also has immutable `standing` or `eligible` kind, not a pinned role-policy period. Its own current identity/revision and time/revocation facts remain authoritative. Current role policy determines kind compatibility: standing contributes only under standing policy; eligible contributes no permission and sources activation only under activation-required policy. Same-mode activation-policy edits preserve eligibility but invalidate old policy-bound requests/windows. Mode changes never convert assignments; every mode change or return requires the same exact complete non-revoked affected-assignment manifest used for permission broadening. Include scheduled and naturally expired-but-unrevoked assignments for stable completeness, and read kind/time bounds from their revision-checked protected rows instead of duplicating those fields in the manifest. A combined change reuses one manifest. See [policy compatibility](groups-and-privileged-access.md#privileged-classification-and-policy).

The initial Group, membership, assignment, activation and delegation facts use protected current rows with monotonic revisions and trusted creation/current-change evidence, not an immutable history table for every fact. Exact source identity plus expected current revision prevents a restored source from reviving an old activation. Immutable role and activation-policy history remain because their exact historical snapshots are required provenance. Full human request/review history stays in ordinary IAM records and later Activity; neither becomes a second access authority. Bounded delegation stores only its current exact protected permission set. Group retirement is terminal in this initial model; membership restoration is a separate revisioned change and requires a fresh activation. No nested Group or second temporary membership engine is introduced.

The [Group-change contract](../../../contracts/src/organization-group-changes.ts) distinguishes creation, label revision and terminal retirement. Creation supplies the organisation, new identity, immutable key and label; revisions and retirement supply the exact reviewed Group revision. Trusted actor/correlation evidence accompanies each command, but no caller timestamp, membership payload or permission decision does. The private composition returns the actual stored Group, operation, resulting Access version and correlation, with exactly one existing Group-related Access increment per successful change. It leaves related facts unchanged, refuses stale or unchanged requests without partial effects and introduces no new fingerprint or approval step. See the [Group lifecycle](groups-and-privileged-access.md#creating-renaming-and-retiring-groups) for its user-facing effect and later protected invocation boundary.

The [membership-change contract](../../../contracts/src/organization-group-membership-changes.ts) defines add, remove, restore and renew. Add supplies a new identity, same-organisation Group/account and fixed window. Remove and restore supply the reviewed membership revision without changing that window. Renewal supplies an expired non-revoked predecessor's reviewed revision, a distinct replacement identity and new window; the writer derives the same Group/account. Its result contains the actual new membership and complete closed predecessor, both bound to the trusted actor/correlation, plus one Access version. Restore retains the original grant and expiry; renewal preserves the predecessor as revoked evidence. Expiry is checked against actual database time after locks, never a clamped audit timestamp. These private compositions do not grant caller authority or revive old activations; [#40](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40) and the [IAM journey](iam-application.md) remain responsible for authorised invocation.

An assignment's active temporal state means only that its time window is current and it is not revoked; an eligible assignment still grants no permission. Activation evidence retains historical source revision numbers for comparison against the current facts. Storage must not bind those numbers with a foreign key to a mutable current revision, which would prevent legitimate membership or assignment changes. Permanent same-organisation identity constraints remain required.

Membership removal/suspension uses its single inactive `revoked` state. Explicit authorised restoration preserves its permanent identity and original grant provenance, advances the current revision, clears current revocation evidence and updates trusted change evidence. This does not restore an old activation. Role-assignment and delegation revocation are terminal; regrant uses a new identity. See the [initial lifecycle rules](groups-and-privileged-access.md#current-state-checks-and-revocation); do not add a separate suspension state or use time bounds to imitate one.

Each membership, assignment and delegation identity has immutable start/expiry bounds. Restore keeps those bounds and requires no expiry or an expiry still in the future; a future-start membership remains scheduled. Natural expiry renewal closes the predecessor and uses a new membership identity and time window. Later protected operations perform that replacement atomically with the existing Access-version change; storage fixtures alone do not prove that operation.

The initial private storage uses five current-fact tables: Groups, memberships, assignments, activations and delegation. Permanent same-organisation relationships use identity foreign keys. Mutable source revisions remain scalar evidence on activations so later membership/assignment changes are not blocked. Activation insertion binds the then-current role revision, exact policy and authority periods, valid account/eligible source and a nonempty retained accepted permission set, with finite duration bounded by policy and source ends. Later reads retain historical role provenance and do not equate it with the latest revision. These integrity checks do not verify the human caller, approval or authentication; [protected operations #40](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40) must do so before exposing activation.

Bounded delegation keeps its current exact permission tuples in one closed, deterministically ordered JSONB array on the delegation row. Validate unique exact identities, complete keys/types, immutable catalogue/registration evidence and current permission continuity when that set is inserted or explicitly replaced. Replace the entire set as one row; no child-row sealing protocol or separate identity/state framework is needed. The scope fingerprint is provenance, not a substitute for these tuple checks: protected preparation derives it, and the central decision checks the actual current tuples and continuity. This use follows PostgreSQL's guidance to keep a [JSON document an atomic, predictable unit](https://www.postgresql.org/docs/17/datatype-json.html#JSON-DESIGN).
...
Revocation names the exact assignment and expected current assignment revision. It retains the original role, holder, kind, window and grant evidence, and remains possible after the holder or role becomes inactive. Grant or revocation and one `role_assignment_changed` Access-version increment commit together; duplicate, stale, already-revoked or exhausted changes do not partially write or increment. These initial owner-only compositions are not authority gateways. Before [#40](https://github.com/Abzum-NZ/Abzum-Vortex/issues/40) exposes them, the remaining [#33 stewardship safeguards](https://github.com/Abzum-NZ/Abzum-Vortex/issues/33) must participate in the same change and protect the final permanent steward. Do not substitute a partial steward evaluator or a direct grant screen.

```mermaid
flowchart LR
    REVIEW[Exact assignment change and reviewed revision] --> LOCK[Wait for the organisation change lock]
    LOCK --> CHECK[Recheck revision and applicable grant or revocation rules]
    CHECK --> SAVE[Commit assignment and Access version together]
    CHECK --> REFUSE[Refuse stale or invalid change without writing]
```

This diagram describes the private change composition only. The later IAM operation supplies current caller authority and the permanent-steward safeguard before this composition can be exposed to any caller.

The permission catalogue records the active registrations supplying each bound-module permission and their exact owner/release evidence. Removing one supplier preserves availability only where another active compatible supplier remains. Publishing a new definition is inert; only authorised activation, upgrade or withdrawal changes live availability and the Access version. Catalogue availability alone grants no application entry or data scope.
...
The version number, not the timestamp, establishes the strict order of access changes. The atomic increment records `changed_at` as the later of the stored observation time and the database clock at the update. Equal timestamps are allowed: a waiting request or a backwards clock adjustment must not reject an otherwise valid increment or move its observation time backwards. Keep the existing one-increment and nondecreasing-time constraints. This timestamp is not a clock for eligibility, expiry, recent authentication or permission decisions; those retain their own live-time checks. See [PostgreSQL time-function semantics](https://www.postgresql.org/docs/17/functions-datetime.html#FUNCTIONS-DATETIME-CURRENT).
...
A calculation field uses one closed expression kind: join text fields, apply a numeric operation over named field/literal operands and further numeric operations, evaluate a typed condition, offset a date/date-time, or determine whether a deadline has passed while excluding explicitly listed terminal states. Its declared result type must match the expression kind and its field dependencies are explicit. A total uses one fully qualified `module:record_type.relationship` source relationship, one closed aggregate operation, an explicit result type, an optional compatible field, and an optional typed condition. The relationship must target the total-owning record; aggregate fields and filter operands resolve in the relationship's source record. Count is a whole number; sum, minimum and maximum preserve the compatible source-field type; average is decimal or money. Neither contract accepts an arbitrary expression or filter object.

A relationship names one source field and either one target record type or an explicit polymorphic target list. This lets a single logical link-to-one-of-several relationship retain one stable relationship identity, including when inherited ownership or totals refer to it. A link to a person or a group is an ordinary `link` field whose target is the People or Group system record type; the optional application-access requirement is a link setting accepted only for a People target, and a well-formed target identifier alone proves neither existence nor access.

The twenty-one type keys and their settings are defined in [Modules, fields and relationships](../05-modules-fields-and-relationships.md#field-types). Attachment settings are defined only in [Files and attachments](../11-files-and-attachments.md#canonical-attachment-settings). Unsupported common properties and unknown type settings are refused.
...
### Extension fields on system record types

A [system record type](core-contract-boundary.md#system-modules) is the read-only People, Group or organisation-profile projection of protected Identity and Access storage. Its system module declares an ordinary extension point on each of these system record types; the system record type is extended only through that extension point, and a contributed field uses the ordinary field contract. Contributed values are stored as ordinary organisation data in a companion record keyed one-to-one to the system record identity, never in the protected storage and never as a change to the protected fact. They live in the contributing organisation only, are written through the record-change command under the contributed field's own permissions, and are available to lists, filters, rules, queries and access conditions exactly like an ordinary field. Removing a contribution hides the field and preserves its stored values under the ordinary retention policy. Standard create, update and delete on the system record type remain refused, and a contributed field never widens the protected projection's storage or its protected-operation write path.
...
The runtime record exposes ownership as the union above. Physical record tables provide nullable protected organisation-account and group owner identifiers plus a constraint derived from the published ownership mode: neither for none or inherited ownership, exactly the matching one for organisation-account or group ownership. An inherited record resolves its owner and access through its declared parent relationship rather than copying the parent owner.
...
```mermaid
flowchart LR
    UI[Forms and flow tasks] --> C[Shared save command]
...
The recipient stores one non-content mirror with `grant_id`, source and recipient cluster/organisation/application identifiers, approved recipient region, recipient role identifiers, contract version and fingerprint, definition-mapping fingerprint, source route, state, source proposal fingerprint, local access-version contribution, last reconciliation time, and safe last outcome. Evidence depends on state: pending carries the proposal and may carry a recipient decision but no activation receipt; active requires the recipient decision and signed activation receipt; suspended adds signed suspension evidence; revoked or expired carries signed state evidence and carries the activation pair only when activation had occurred. It stores no source record fields, files, query results, search documents, or flow payloads. The source grant remains authoritative when the mirror disagrees or is stale.

## Action, flow and condition contracts

User-facing action and data-component bindings reference an exact application-owned flow, event identity and typed input/context map; they do not contain an alternate operation handler. Every flow has exactly one owner, the module or application release that contains it, so there are no platform-managed or ownerless flows; behaviour that must stay hidden is a protected operation, and reusable platform behaviour ships as ordinary flows in system modules. A flow may terminate with presentation only or sequence registered pure, read, change, interface and background-start tasks. Each protected operation commits or refuses atomically in its own short transaction; later failure preserves earlier commits. The default Save is a generated one-task `record.save` flow, not a separate kind of flow, and collect-first remains an optional authoring pattern. Default flow identities are allocated during revision-checked authoring, not rendering. [Bindings delivery](https://github.com/Abzum-NZ/Abzum-Vortex/issues/250) owns these reference shapes, and [flow delivery](https://github.com/Abzum-NZ/Abzum-Vortex/issues/58) owns their shared validation and execution. Use explicit current flow bindings without legacy direct-binding readers or conversions. See [page-to-flow composition](frontend-rule-designer.md#pages-compose-flows-define-actions).

A **named action** is a `transaction` flow in the one [flow language](frontend-rule-designer.md#one-flow-language). It contains identifier, key, label, subject record type, permission binding, sharing setting (`refused` by default or `allowed`), uniquely keyed typed inputs and a precondition. Each input has a label and required flag. Its validation contract is discriminated by type: plain/formatted text, number, Boolean, date, date-time, one-or-more record-type reference, or organisation-account reference. One input cannot accept another type's settings. Its ordered task list may contain only transaction-safe tasks, and its record tasks compile into one apply-record-changes call: `record.setFields`, `record.create`, `record.link`, `record.delete` and an **Announce event** task. A **Link records** task names every source relationship and its target-record input; it never means “all relationships.” A shared action executes wholly in the source organisation and cannot create a relationship to recipient-owned data.
...
The first implementation profile is the current rule graph's `before_save`
profile, embedded in the explicit Module source/validation pair `3.0.0`. It uses
the current exact-value field values and its Start, Condition, Set variable, Set
field, Require field, Warn, Refuse and Finish nodes, which become a `BeforeSave`
flow with the trigger, **If**, **Set variable**, **Set fields** (the record being
saved only), **Require field**, **Warn**, **Refuse save** and **Stop** tasks under
the [one flow language mapping](../08-forms-actions-rules-and-events.md#one-flow-language-mapping). Typed run-local variables are not infrastructure
environment variables or secrets. Applicable rules run in ascending priority and permanent
rule-ID order. **Require field** tasks return checks for the owning Record save's
final candidate, after all applicable rules and owning generators. Definition carries the exact pair through compilation, semantic validation,
publication, consumer reads and restore, retaining task, input and variable identities
inside their permanent rule owner. The protected Record save invokes the Rule interpreter;
parsing or publishing a flow alone does not execute it. The other interactive and
effectful tasks remain required extensions of this same engine, not a second
designer or interpreter.

Current rule definitions express create/change triggers, recursive conditions, permanent rule identity, priority, field assignments, requirements, warnings and refusals directly. Keep safe message codes and final-candidate validation. Remove old-format conversion operations and align checked-in definitions with the current schema.
...
### Definition history and restore contract

The exported history-list command is strict and kind-discriminated. It contains `kind`, the matching branded Module or Application `rootId`, `pageSize` from 1 through 100, and optional JavaScript-safe positive `beforeReleaseRevision`. The exact-entry command contains the same kind and root plus one JavaScript-safe positive `releaseRevision`. Unknown, foreign and wrong-kind roots share one root-not-found outcome; an existing root with no releases returns an empty first page.

History uses newest-first revision keyset pagination. A continuation reads revisions strictly lower than `beforeReleaseRevision` and returns `nextBeforeReleaseRevision` only when another entry exists. The root pointer and every `isCurrent` value are derived in the same database statement. Each metadata entry contains exactly `releaseRevision`, stable `releaseVersion`, `sourceFingerprint`, `contentFingerprint`, required `releaseNote`, `publishedAt`, `publishedBy` and `isCurrent`. The containing result also identifies `kind`, `organizationId`, `definitionKey`, `rootId`, optional `currentReleaseRevision`, optional continuation and `correlationId`. It excludes authored source, canonical content, compilation output, resolution snapshot, dependency manifest, impact reasons, comparison evidence and draft state.

The strict restore command contains only `kind`, the matching branded `rootId`, `targetReleaseRevision` and `expectedDraftRevision`. It cannot carry authored source, a fingerprint, identity requirement, actor, time, correlation evidence, release version or pointer state. The service selects the immutable authored source itself, validates its source-contract version and source fingerprint, verifies the complete stored release through the shared release-integrity rules, and re-derives current source-identity requirements. Every required identity and current alias must already exist, match the same source owner and agree with the release's immutable identity snapshot. Restore allocates nothing.

A successful restore increments the draft revision once and returns the strict stored draft with all five provenance values present: `restoredFromReleaseRevision`, `restoredFromSourceFingerprint`, `restoredBy`, `restoredAt` and `restoreCorrelationId`. Database storage binds the first two values to the exact `(root_id, release_revision, authored_source_fingerprint)` release snapshot and requires the complete group to be all present or all absent. A successful ordinary draft save clears the group; a stale or failed save or restore changes nothing.

Stable safe failures cover invalid commands, root or release absence, stale or missing draft, context refusal, immutable release or identity-integrity failure, invalid stored result and unexpected operation failure. Unknown, foreign, wrong-kind and unknown-revision restore targets are indistinguishable. No failure returns stored source, refused-row fingerprints, raw SQL or another organisation's existence.

## Application, page and block contracts

An application contains root identity and its own release version, exact resolved module/version bindings, application permission declarations and roles, navigation, pages, actions, rules, events, flows, process pipelines, theme settings or platform-theme binding, version-pinned connection bindings, interfaces, public addresses, and default states.

A process pipeline contains one record type and stage field, uniquely keyed stages with labels and explicit entry and exit actions or flows, named transitions with optional permission/action and typed gate, and time targets with a stage, date-time field and escalation event. Every stage, field, action, flow, permission and event reference resolves before publication.

The normative page, shell, slot, setting, theme, responsive and binding contracts are specified in [Page builder contracts](page-builder-contracts.md). The current implementation's flat placements, desktop/phone-only geometry and blanket same-record-type check require the coordinated completion in [#249](https://github.com/Abzum-NZ/Abzum-Vortex/issues/249) and [#250](https://github.com/Abzum-NZ/Abzum-Vortex/issues/250).

Pages and shells have stable contained identities and publish with their Application. Placements form an explicitly ordered tree through registered slots; duplicate identities, cycles, orphan slots and incompatible children fail validation. Literal values satisfy registered recursive property schemas. References and query/action parameters resolve against explicit page, relationship or row contexts. Forms bind typed input schemas to one owning operation with concurrency, confirmation and duplicate protection.

Desktop, tablet and phone layout inheritance, content-driven height, theme tokens and permitted responsive order are explicit. Old immutable page releases retain supported readers; conversion occurs only through a deterministic new-draft migration.

Public pages retain their stricter field/action allowlist. Shared-record panels retain source-authoritative access and cannot create cross-source joins.
...
<a id="workflow-execution-reference-and-protected-operation"></a>

## Flow execution reference and protected operation

A Vortex flow reference has `run_id`, tenant, organisation, application and application version, flow identifier and contained version, trigger and source identifier, start actor, duplicate-protection key, human-input links, safe activity links, last refresh time, and a clearly non-authoritative last-known state snapshot. A published flow trigger contains unique typed inputs, a nullable typed condition, and an explicit duplicate-protection rule. Event inputs resolve to carried fields. A schedule trigger owns its closed cadence, interval, time zone and cadence-specific clock fields; it cannot name an undeclared external schedule. Every other non-event input resolves by payload key, type and allowed record target against the exact action, incoming-message shape or interface operation that owns the trigger; trigger kinds whose contract declares no inputs must publish an empty input list. A parent flow starts a child only through a Run flow or Run background flow task that names the exact child flow and maps its declared typed inputs; a child receives a current record only through such a typed input. A trigger-input value names only one declared local input. Every fixed task output declares its key, value type and record-target source where applicable; a form task declares its dynamic outputs and allowed record targets in its task definition. Link-field assignments retain target record types, while file tasks require a file reference and an attachment field. The Workflow Kestra adapter stores its Kestra execution mapping privately; provider-specific identifiers are not part of the canonical application contract.

The target application side-effect receipt has run, task, attempt, duplicate-protection key, accepted time, safe input fingerprint, outcome, and resulting record/action/event identifiers. It proves duplicate safety but does not become the flow-state authority. The protected Record and Query execution/receipt runtimes remain future work owned by [#47](https://github.com/Abzum-NZ/Abzum-Vortex/issues/47) and [#54](https://github.com/Abzum-NZ/Abzum-Vortex/issues/54); the delivered Activity foundation is not that receipt runtime.
...
The response is `completed`, `already_completed`, `waiting`, `retryable_failure`, or `permanent_refusal`, with a stable safe code and optional next poll time. Kestra's execution API is authoritative for queued, running, waiting, completed, cancelled, and failed durable flow status.
...
| Tasks in one durable flow      | 1–100                                              |
| Durable flow nesting depth     | 5                                                  |
| Records in one durable repeat  | 1,000, processed in pages                          |
...
An incoming message has organisation, connection, message type, provider message identifier or fingerprint, verified time, safe payload reference, duplicate state, flow trigger result, and retention due time.

An interface operation has interface and major version, operation key, explicit HTTP method and application-relative path, target-aware input/output shapes, authentication method, required permission, visibility, rate/size limits, duplicate-protection rule, the called flow entry point (a named action or an interface-started flow) or query, and stable error catalogue. An interface operation never binds a lower-level operation directly. Each shape field declares type, required status, and its exact target binding. A named-action entry point requires one subject binding and bindings for all required declared inputs, with no output fields. A query has no inputs and returns at least one selected field, plus optional standard page information. An interface-started flow has no inputs and returns exactly its run identifier. Standard record tasks, query parameters, action results, and flow inputs remain unavailable until their owning contracts explicitly define those values. Publication resolves every binding and checks its type. A public operation refuses administrative permissions and non-shareable actions; its action condition, subject values, changed fields and created-record values may use only fields approved for public display on the correct record type. Public query outputs receive the same field approval check. An interface dependency records minimum and maximum accepted major/minor versions.
...
A **semantic control entry** has a stable control identifier, control kind, plain label and description, containing page/region, current availability, optional safe unavailable reason, typed input or value contract, confirmation or reauthentication requirement, exact target flow binding, duplicate-protection rule where changing, and result shape. The server retains the required permission in the internal flow binding but never projects that permission key to the client. A discoverable unavailable entry appears in the semantic resource but not the invocable tool choices. Control kinds cover navigation, page/record opening, view-state selection, query/filter/sort/page/refresh, form draft and guided step, named action, file operation, builder change, publication and administration. A semantic form entry additionally has form and draft identifiers, current draft revision, field order, write-only markers, allowed choices, validation results and commit flow.

An **MCP tool request** identifies the interaction context, protocol revision and client capabilities, semantic control, expected semantic-state or draft revision, typed input and duplicate-protection key where required. The result is the ordinary bound flow outcome plus correlation/activity references and the new state, draft or record revision. MCP protocol failures remain distinct from application validation or access refusals; application results use the same stable safe codes as the web interface.
...
- A versioned lifecycle policy binds organisation, storage contract and optional contained application scope; age/count limits, permitted end action and exact registered flow/destination references are validated against current organisation limits. Scope is never an untrusted table name. The same shared row set has one policy.
