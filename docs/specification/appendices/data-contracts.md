...
User-facing action bindings reference an exact application-owned Frontend Flow, event identity and typed input/context map; they do not contain an alternate operation handler. A flow can terminate with presentation only or at most one final protected business submission. Save form is a one-node convenience over the exact form commit operation. Default flow identities are allocated during revision-checked authoring, not rendering. [Bindings delivery](https://github.com/Abzum-NZ/Abzum-Vortex/issues/250) owns these reference shapes, and [rule delivery](https://github.com/Abzum-NZ/Abzum-Vortex/issues/58) owns their graph validation/execution. Preserve immutable legacy direct bindings through their supported reader; new-draft conversion materialises explicit flows. See [page-to-flow composition](frontend-rule-designer.md#pages-compose-flows-define-actions).
...
Interactive Show form bindings declare exact form/flow/node identity, typed defaults and response output maps. One private revision-checked draft collects answers and proposed action inputs. The journey executes at most one bounded owning operation when it declares one; that final submission revalidates the mandatory path and commits atomically. Input-only forms need no owning operation and their response alone is not a business commit. Browser variables and node-path claims are not authority. [Bindings #250](https://github.com/Abzum-NZ/Abzum-Vortex/issues/250) owns these contracts, including explicit record-free input forms and protected workflow-start input maps; the [page contract](page-builder-contracts.md) remains the sole form representation.
