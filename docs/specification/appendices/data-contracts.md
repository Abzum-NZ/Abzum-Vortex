...
Complete role-entry sets use the final immutable role revision as their seal. The private writer inserts entries before that revision through a deferred reference. Every entry insertion requires and locks the permanent role and refuses if the target revision already exists, preventing late or concurrent additions. The final revision validates the whole set once; entry updates/deletes remain forbidden. All owning operations take the organisation governance lock before role locks. This needs no separate seal record, receipt framework or mutable approval flag, and the accepted-grant fingerprint remains acceptance provenance rather than a repurposed storage checksum.
