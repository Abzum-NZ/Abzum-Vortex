...
Organisation-owned roles may select exact permissions from several registered applications or modules in that organisation. A custom role made from an application template is independent live organisation data and is not overwritten by later application releases. Registration creates no account or Team assignment. Existing assignments do not receive newly added or broadened application permissions without an explicit authorised review; removed permissions are unavailable. The owning application change and its Access-version increment must be atomic.
...
The Access service owns an `access_version` per organisation. Every access-affecting organisation-account, role, assignment, team, sharing, public-policy, or application-access change increases it in the same transaction as the owning state change.
