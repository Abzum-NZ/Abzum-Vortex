...
flowchart TD
    ...
    ID[Identity and membership records] --> ACCESS[Access decision]
...
`organisation_id`, permanent `short_name`, `display_name`, `state`, `state_changed_at`, `owner_account_id`, `created_at`, and `created_by`.
...
`account_id`, verified primary email, display name, state, second-factor enrolment state, optional language and time zone, creation time, and last successful sign-in time. Credentials and second-factor secrets are stored only by the identity provider boundary, not in the profile record.

### Membership

`membership_id`, `organisation_id`, `account_id`, state, invitation details, activation/suspension/withdrawal times, and access-version contribution. Group/team membership shape follows [Decision D23](decisions.md#d23-groups-and-teams).

### Invitation

`invitation_id`, organisation, normalised invited email, proposed role assignments, one-way token fingerprint, created/invited/expiry/revocation/acceptance times, inviter, and accepted membership.

### Session context

Person or system actor, organisation, application where present, membership, caller kind, session and issue times, expiry, authentication strength, access version, correlation identifier, and delegated/support context where present.
...
A role assignment has organisation, role, assignee kind, assignee identifier, optional application, start and expiry times, state, grantor, and activity link. Assignment to a team uses the team model selected in [Decision D23](decisions.md#d23-groups-and-teams).

The Access service owns an `access_version` per organisation. Every membership, role, assignment, team, sharing, public-policy, or application-role publication change increases it in the same transaction.
...
Storage scope is `organisation_shared` or `application_contained`. Ownership mode and record sharing depend on [Decision D24](decisions.md#d24-individual-record-sharing).
...
| `owner_account_id` | Present when ownership is enabled. |
