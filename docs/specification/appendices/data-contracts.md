...
The closed reasons distinguish standalone `role_catalogue_changed`, `role_assignment_changed` and `role_activation_changed`. Activation changes an individual use window without changing its eligibility assignment. Its reason has the same spelling in current and stored contracts; the existing historical Group-name conversion is unchanged. A coordinated application catalogue/continuity/role change uses `application_access_changed` once for the whole transaction, not another increment per changed role. A refused or unchanged replay increments nothing. Add new reasons to the existing contract and database allowlist without rewriting old change records or creating another counter.
