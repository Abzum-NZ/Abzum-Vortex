...
| 20. Connections to other systems | [Connections, programmable interfaces and MCP](../12-connections-and-interfaces.md) | OAuth lifecycle, request forgery protection, retries, redaction, and inbound replay supplied. |
| 21. Application interface and assistant | [Connections, programmable interfaces and MCP](../12-connections-and-interfaces.md), [Product boundaries](../01-purpose-and-scope.md#product-boundaries) | Customer-published interfaces remain, and governed MCP parity lets an authorised external client use the same capabilities as the person. A Vortex-hosted model, assistant, sampling request and autonomous execution remain outside the release. |
