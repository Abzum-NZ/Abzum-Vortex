...
The [Frontend Rule Designer](appendices/frontend-rule-designer.md) uses the same typed condition and value concepts, but immediate evaluation runs in Vortex. Its Show form node collects private draft answers before final submission; the durable request_form node below parks a Kestra execution after work has been accepted. Both reuse the Page Designer form renderer. They do not keep a database transaction open while awaiting a person.
