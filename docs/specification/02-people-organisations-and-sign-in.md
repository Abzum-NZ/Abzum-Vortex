...
The private tenant and organisation tables contain structural identity and lifecycle facts only. Protected provisioning, hierarchy commands, tenant-administrator assignments, the read-only system record types that expose those protected facts, expected-revision command concurrency, duplicate protection, and activity evidence sit above those tables in the Identity service. Applications read those facts only through those system record types over the one [query path](10-queries-reports-search.md#one-read-path), under the viewer's current authority, as described in the [core contract boundary](appendices/core-contract-boundary.md#system-modules). Provisioning supplies explicit values to the Identity-owned runtime-settings initializer from [#430](https://github.com/Abzum-NZ/Abzum-Vortex/issues/430); that task owns the settings validation, storage, updates and the internal runtime reader the server uses to apply settings, which is never an application read path. Neither layer introduces a hardcoded administration page.
...
Protected tenant-governance operations use a server-resolved verified identity and current tenant-administrator assignment; they do not require an active account inside the target organisation. Otherwise an administrator could not create the first organisation or restore a suspended organisation. The Identity service validates the selected tenant and target, the current assignment, expected revision and each lifecycle transition inside its protected transaction. This narrow tenant context cannot read organisation records or be used as an organisation request context. System-only first provisioning remains separate and idempotent. Organisation-local application data and account operations still require their documented organisation authorization path. Administrative account, invitation, and runtime-settings reads are reads of their read-only [system record types](appendices/core-contract-boundary.md#system-modules) through the one [query path](10-queries-reports-search.md#one-read-path). Each requires the exact organisation `.read` permission, the current active [#27](https://github.com/Abzum-NZ/Abzum-Vortex/issues/27) request context, and the [#34](https://github.com/Abzum-NZ/Abzum-Vortex/issues/34) decision, and returns only bounded safe local projections; tenant authority, membership, another read permission, and `.manage` authority cannot substitute. The runtime-settings system record type projects the settings owned by the [#430](https://github.com/Abzum-NZ/Abzum-Vortex/issues/430) initializer rather than duplicating settings behaviour. [Issue #30](https://github.com/Abzum-NZ/Abzum-Vortex/issues/30) owns this boundary; it does not weaken active-account entry checks or expose private Identity tables.
...
Tenant Administration and Organisation Administration are [system applications](07-applications-pages-and-themes.md#system-applications), platform packages that are installed when an organisation is created and cannot be uninstalled. They use ordinary modules, records, pages, roles and workflows while calling narrowly protected identity, hierarchy, access, entitlement and data-handling operations, and their pages may require exact [platform administration permissions](appendices/platform-permission-catalogue.md) in addition to their application permissions. Their protected operations and operation bindings stay platform-owned. The engine does not contain special portal page logic. Like every application, each is reached through its own definition-led application address rather than a reserved administration route, and its administration agent tools come from its own application tool bundle.
