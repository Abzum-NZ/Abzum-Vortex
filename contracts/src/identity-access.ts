 ...
 * deliberate interchange values; deprecated aliases such as Asia/Calcutta are
 * not accepted.
 */
const supportedIanaTimeZonesV2026d = new Set<string>([
  "UTC",
  "Etc/UTC",
  "Africa/Abidjan",
  "Africa/Accra",
...
const canonicalLanguageTagSchema = z
  .string()
  .min(2)
  .refine((value) => {
    try {
  ...
  }, "Language must be a canonical BCP-47 language tag");
...
export const organizationRuntimeSettingsSchema = z
  .object({
    ...
    timeZone: z
      ...
      .min(1)
      .refine(
        (value) => supportedIanaTimeZonesV2026d.has(value),
        "Time zone must be a supported canonical IANA zone",
      ),
    currency: z
      .string()
      .length(3)
