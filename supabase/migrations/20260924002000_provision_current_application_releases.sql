-- #830: provision_module_installation_storage still required Application
-- validation contract 1.0.0, while publication compiles every Application at
-- 2.0.0 and activation/detach (20260913010000) already require 2.0.0. Every
-- newly published Application was therefore refused at provisioning and could
-- never be installed. This re-creates the latest definition
-- (20260911090000_resolve_reachable_module_dependencies.sql) unchanged except
-- for the exact Application release version it accepts.
...
begin
  ...
  if application_release.validation_contract_version <> '2.0.0'
    ...
    or application_release.compilation_output #>> '{validationContractVersion}' <> '2.0.0' then
