-- #609: organisation provisioning runs the whole platform catalogue chain (1.0.0 to 1.4.0) at
-- creation, and the role and registration changes made through request entries, which no earlier run
-- exercised. These defects stopped them:
--
--   * revise_platform_permission_catalogue_metadata (revision 1 to 2) left every platform
--     permission continuity at last_processed_registration_revision 1, but the next step,
--     adopt_shipped_platform_permission_catalogue (revision 2 to 3), requires the continuities
--     to be processed at revision 2. The metadata revision now advances them; no meaning changes.
--   * platform_permission_catalogue_revision_is_exact_v1_2_0 declared a constant named
--     meaning_fingerprint and compared entry.meaning_fingerprint = meaning_fingerprint, which
--     PL/pgSQL rejects as an ambiguous column reference. The constant is renamed.
--   * Three deferred constraint-trigger validators (role revision evidence, permission continuity
--     evidence, application role-template continuity evidence) ran as security invoker. A deferred
--     trigger fires at commit under the transaction's current role, which for every request
--     transaction is vortex_request and holds no table privileges, so any role or registration
--     change made through a request entry failed at commit with "permission denied for table".
--     They only read evidence and raise, so they are now security definer.
--
-- Each function is installed as its complete body, identical to its canonical file under
-- supabase/schemas/vortex_access/, with the comment and privileges it already had.

begin;

create or replace function vortex_access.revise_platform_permission_catalogue_metadata(
...
declare
  ...
  current_catalogue_fingerprint constant text :=
    'sha256:94733e5bba0c59d8b81693c78711b34f87bc7ff82c68f705d343cdacda990058';
...
begin
  ...
  -- A metadata-only revision leaves every permission meaning unchanged, so each platform
  -- continuity is processed against the new registration revision. The chained organisation
  -- initializer adopts the next catalogue at this revision and requires that evidence.
  ...
  where continuity.organization_id = p_organization_id
    ...
    and continuity.registration_owner_id = 'cabe121e-0baf-4084-9471-cce915d460a8'::uuid;
