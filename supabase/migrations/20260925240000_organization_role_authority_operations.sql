-- #1055: protected create-custom-role and accept-role-template request entry.
-- It reuses the canonical owner-only role-authority composition so the
-- affected-assignment review, explicit acceptance evidence and permanent-steward
-- safeguard are unchanged. The composed operation writes the Activity itself and
-- this entry only projects the stored role summary for the administration service.
-- The raw composition stays owner-only; the request role receives only this
-- checked, authority-establishing entry point.
