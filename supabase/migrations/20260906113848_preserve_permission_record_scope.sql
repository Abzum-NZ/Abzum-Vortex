-- Preserve optional Definition-owned record scope through the existing private
-- permission catalogue without changing historical absent declarations.

alter table vortex_access.permission_catalogue_entries
  add column record_scope jsonb;

alter table vortex_access.permission_catalogue_entries
  add constraint permission_catalogue_entries_record_scope_shape check (
    record_scope is null
    or (
      registration_kind = 'application'
      and record_type_id is not null
      and pg_catalog.jsonb_typeof(record_scope) = 'object'
    )
  );

-- Preserve nondecreasing permission-registration audit evidence when a
-- revision-checked writer waits for the shared organization governance lock.
  ...
  loop
    ...
    if entry_owner_kind not in ('application', 'module')
      ...
      or permission_value - array[
        ...
        'recordScope', 'actionKind', 'namedAction', 'administrative'
      ]::text[] <> '{}'::jsonb
...
comment on function vortex_access.read_available_permission(uuid, uuid, text, uuid, uuid) is
  'Owner-only exact current permission lookup retaining application context and optional record scope.';
