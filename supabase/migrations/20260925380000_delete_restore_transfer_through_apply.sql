-- #1064: route delete, restore and ownership transfer through apply_record_changes.
--
-- The protected recoverable delete, the policy-bound restore and the single-record
-- ownership transfer were three writer pairs (prepare/finalize delete, prepare/
-- finalize restore, and transfer_record_ownership) with their own entry points,
-- receipts and Activity helpers. This migration folds their terminal writes into
-- the one protected apply_record_changes operation, so the request role reaches
-- every record change through it:
--
--   * apply_record_changes gains a lifecycle branch for the delete, restore and
--     transfer_ownership operations. The branch delegates to
--     apply_lifecycle_record_changes_internal, whose three branches are the exact
--     terminal bodies the retired writers carried, so every receipt, fingerprint,
--     Activity and Event is unchanged.
--   * apply_lifecycle_record_changes is the request role's closed entry: it
--     accepts only those three operations and delegates once, so an ordinary save
--     or named action cannot skip its own preparation by calling the operation
--     without one.
--   * finalize_protected_record_delete, finalize_protected_record_restore and
--     transfer_record_ownership are dropped. Their protected preflights
--     (prepare_protected_record_delete and prepare_protected_record_restore) stay,
--     because the Record runtime still derives the relationship-total closure
--     from the preflight before the terminal write.
--   * transfer_offboarding_active_owned_record delegates to the same terminal
--     ownership transfer, so the offboarding rule keeps its exact behaviour.
--
-- Each function below is installed with its unchanged signature, security and
-- search_path where it existed, and its statement is identical to the canonical
-- file supabase/schemas/vortex_record/<function>.sql changed in this commit.
...
-- The offboarding entry revoked the adapter's own EXECUTE, so create or replace
-- cannot replace it in place; drop it first and install the new complete body.
drop function if exists vortex_record.transfer_offboarding_active_owned_record(
...
drop function vortex_record.finalize_protected_record_delete(uuid, uuid, uuid, bigint, jsonb);
drop function vortex_record.finalize_protected_record_restore(uuid, uuid, uuid, bigint, jsonb);
drop function vortex_record.transfer_record_ownership(uuid, uuid, uuid, bigint, text, uuid, uuid, uuid);
