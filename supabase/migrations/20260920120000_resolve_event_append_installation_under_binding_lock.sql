-- #508: Event append must not emit occurrences using Application installation
-- evidence that a committed lifecycle detach has already invalidated.
--
-- #475 hoisted the installation read out of `vortex_event.append_record_occurrences`
-- into the public wrapper, ahead of the canonical binding lock that the common
-- writer takes, and the writer then trusted the passed value. In read
-- committed, the wrapper's read fixed its snapshot before the lock was waited
-- on, so a `vortex_module.detach_application_installation` that committed
-- during the wait was never observed and the append emitted the pre-detach
-- binding. The read moves back inside the protected region, restoring the
-- pre-#475 order (lock, then read, as separate statements of a volatile
-- function).
--
-- `vortex_event.append_detached_offboarding_reassignment_internal` is
-- unchanged: `vortex_module.read_current_detached_installation_for_transfer_internal`
-- takes the exclusive canonical binding lock itself and reads afterwards, so
-- its pin-set is already resolved inside a protected region and is still
-- passed in.
...
-- Event retains the established active-only public boundary.  The active
-- installation is resolved by the common writer under the canonical binding
-- lock, never here: a read taken in this wrapper would fix its snapshot before
-- that lock is waited on.
