...
begin
  ...
  if p_registration_revision = 1 then
    ...
    select pg_catalog.jsonb_agg(
      case item.value ->> 'key'
        when 'platform.organization.teams.read' then item.value || pg_catalog.jsonb_build_object(
          'label', 'View teams',
          'description', 'View the selected organisation''s Teams and membership administration data.'
        )
        when 'platform.organization.teams.manage' then item.value || pg_catalog.jsonb_build_object(
          'label', 'Manage teams',
