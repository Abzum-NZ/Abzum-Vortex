-- Preserve optional immutable Definition-owned field policy through the existing
-- private permission catalogue without changing historical absent declarations.

alter table vortex_access.permission_catalogue_entries
  add column field_policy jsonb;

alter table vortex_access.permission_catalogue_entries
  add constraint permission_catalogue_entries_field_policy_shape check (
    field_policy is null
    or (
      registration_kind = 'application'
      and record_type_id is not null
      and pg_catalog.jsonb_typeof(field_policy) = 'object'
      and field_policy - array['readableFieldIds', 'changeableFieldIds']::text[] = '{}'::jsonb
      and field_policy ?& array['readableFieldIds', 'changeableFieldIds']
      and pg_catalog.jsonb_typeof(field_policy -> 'readableFieldIds') = 'array'
      and pg_catalog.jsonb_typeof(field_policy -> 'changeableFieldIds') = 'array'
    )
  );

create function vortex_access.permission_field_policy_is_valid(p_policy jsonb)
returns boolean
language plpgsql
immutable
security invoker
...
begin
  if p_policy is null
    ...
    or p_policy - array['readableFieldIds', 'changeableFieldIds']::text[] <> '{}'::jsonb
    or not (p_policy ?& array['readableFieldIds', 'changeableFieldIds'])
    or pg_catalog.jsonb_typeof(p_policy -> 'readableFieldIds') <> 'array'
    or pg_catalog.jsonb_typeof(p_policy -> 'changeableFieldIds') <> 'array' then
    return false;
  end if;

  previous_field_id := null;
  for field_value in
    select item.value from pg_catalog.jsonb_array_elements(
      p_policy -> 'readableFieldIds'
    ) as item(value)
  loop
    if pg_catalog.jsonb_typeof(field_value) <> 'string' then return false; end if;
    field_id := field_value #>> '{}';
    if not vortex_context.is_non_nil_uuid(field_id)
      or (
        previous_field_id is not null
        and previous_field_id >= pg_catalog.lower(field_id)
      ) then
      return false;
    end if;
    previous_field_id := pg_catalog.lower(field_id);
  end loop;

  previous_field_id := null;
  for field_value in
    select item.value from pg_catalog.jsonb_array_elements(
      p_policy -> 'changeableFieldIds'
    ) as item(value)
  loop
    if pg_catalog.jsonb_typeof(field_value) <> 'string' then return false; end if;
    field_id := field_value #>> '{}';
    if not vortex_context.is_non_nil_uuid(field_id)
      ...
      or not exists (
        ...
        where pg_catalog.lower(readable.value) = pg_catalog.lower(field_id)
      ) then
      return false;
    end if;
    previous_field_id := pg_catalog.lower(field_id);
...
revoke execute on function vortex_access.permission_field_policy_is_valid(jsonb)
  from public, anon, authenticated, service_role, vortex_runtime, vortex_request;

alter table vortex_access.permission_catalogue_entries
  add constraint permission_catalogue_entries_field_policy_value check (
    field_policy is null
    or vortex_access.permission_field_policy_is_valid(field_policy)
  );
...
create or replace function vortex_access.withdraw_application_permission_registration_v1_internal(
...
begin
  ...
  select entry.organization_id, entry.registration_kind, entry.registration_owner_id,
    ...
    entry.source_catalogue_fingerprint, entry.meaning_fingerprint, entry.record_scope,
    entry.field_policy
...
create or replace function vortex_access.application_permission_registration_matches_candidate(
  p_organization_id uuid,
  p_application_root_id uuid,
...
drop function vortex_access.read_available_permission(uuid, uuid, text, uuid, uuid);
...
revoke execute on function
  ...
  vortex_access.permission_field_policy_is_valid(jsonb),
  vortex_access.read_available_permission(uuid, uuid, text, uuid, uuid)
  from public, anon, authenticated, service_role, vortex_runtime, vortex_request;

comment on column vortex_access.permission_catalogue_entries.record_scope is
  'Optional canonical Definition-owned record scope; historical omission remains SQL null.';
comment on column vortex_access.permission_catalogue_entries.field_policy is
  'Optional canonical Definition-owned field policy; historical omission remains SQL null.';
comment on function vortex_access.permission_field_policy_is_valid(jsonb) is
  'Private strict validator for canonical unique field-policy UUID arrays and their subset rule.';
