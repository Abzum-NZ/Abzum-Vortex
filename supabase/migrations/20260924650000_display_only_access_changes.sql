-- Display-only Access changes (#1070): creating or renaming a Group and
-- revising a role's label or description are single revision-checked writes
-- that leave the organisation Access version, and every cached decision,
-- untouched. Permission, membership, assignment and retirement changes still
-- advance it. Both function bodies below are complete bodies taken from the
-- live definitions.
...
-- The role-metadata request entry now performs the whole display-only revision
-- itself; the separate preparation entry and the prepared-evidence argument no
-- longer exist.
