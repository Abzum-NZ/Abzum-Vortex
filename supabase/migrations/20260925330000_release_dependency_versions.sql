-- #1222: an Application may depend on several exact releases of one platform block, so one
-- page can place Records table 1.0.0 while another places 1.2.0.
--
-- vortex_definition.release_dependencies keyed each stored dependency by
-- (root_id, release_revision, dependency_kind, dependency_reference), and a platform_block
-- reference is its block id, so a second release of the same block was refused as a repeated
-- subject. A platform-block subject is now its block id and exact release version:
--
-- 1. The primary key adds dependency_version. Every other kind keeps exactly one stored release
--    per subject through a partial unique index on the previous key. Every stored row already
--    satisfies both, because the previous primary key was stricter; no stored row changes.
-- 2. append_release refuses a repeated platform-block subject only for the same block and
--    release version, and returns its manifest ordered by release version within one block.
-- 3. read_consumer_release, read_restore_release_evidence and
--    project_consumer_release_evidence order two releases of one block by release version.
--
-- Each function is restated in full from its live definition (including the in-place changes of
-- 20260914240000 and 20260924080000) with only these changes, and is identical to its canonical
-- file under supabase/schemas/vortex_definition. Grants and comments are unchanged. Published
-- releases and their stored dependencies are unchanged.
...
alter table vortex_definition.release_dependencies
  drop constraint release_dependencies_pk;

alter table vortex_definition.release_dependencies
  add constraint release_dependencies_pk primary key (
    root_id,
    release_revision,
    dependency_kind,
...
create unique index release_dependencies_single_release_subject_key
  ...
  where dependency_kind <> 'platform_block';

comment on index vortex_definition.release_dependencies_single_release_subject_key is
  'Every dependency kind except platform_block pins exactly one release per subject in one release.';

-- append_release: a platform-block subject is its block id and exact release version, both
-- when refusing a repeated subject and when ordering the returned manifest.
...
begin
  ...
  expected_history_revision := root_row.current_release_revision;
  ...
  loop
    ...
    if exists (
      ...
      where existing.root_id = p_root_id
        ...
        and (
          supplied_kind <> 'platform_block'
          or existing.dependency_version = supplied_version
        )
    ) then
    ...
    from (
      select
        ...
        case supplied.value ->> 'kind'
          ...
          when 'platform_block' then
            (supplied.value ->> 'blockId') || '@' || (supplied.value ->> 'releaseVersion')
...
-- The three manifest readers order two releases of one block by release version, matching
-- the contract's `platform_block:<blockId>@<releaseVersion>` subject order.
