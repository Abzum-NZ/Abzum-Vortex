...
create table vortex_record.record_deadline_due_metadata (
  ...
  primary key (organization_id, storage_contract_id, record_id)
);
...
begin
  ...
  if p_due_transition is null then
    ...
    insert into vortex_record.record_deadline_due_metadata (
      organization_id, storage_contract_id, record_id, record_type_id,
      ...
      transition_field_id,
      transition_at_value
    ) on conflict (organization_id, storage_contract_id, record_id)
    do update set
      record_type_id = excluded.record_type_id,
      application_root_id = excluded.application_root_id,
