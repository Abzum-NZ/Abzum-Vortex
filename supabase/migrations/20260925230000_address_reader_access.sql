-- Issue #1200: the page-serving runtime role must be able to call the application address reader.
-- The addressed read runs as `vortex_runtime` (runtime/app/src/application-address.ts uses the plain
-- runtime transaction, which does not SET LOCAL ROLE), and #1102 granted it EXECUTE on
-- vortex_module.read_application_address_candidates. The runtime role never had USAGE on schema
-- vortex_module - only vortex_request, vortex_record_adapter and vortex_record_owner do - so the
-- call was denied before the function could run.
--
-- Rather than open the whole vortex_module schema to the runtime role, the reader moves unchanged
-- into vortex_access, which the runtime role already holds USAGE on (20260904112625). Its body,
-- comment and owner are preserved; only its schema changes, and EXECUTE stays limited to
-- vortex_runtime. The caller in runtime/app/src/application-address.ts is updated in the same
-- commit. This mirrors the #579 flow-object decision to place runtime-reachable objects in a schema
-- the runtime role already uses.

drop function if exists vortex_module.read_application_address_candidates(uuid, text, text);
