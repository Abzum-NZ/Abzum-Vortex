-- Preserve nondecreasing role-assignment revocation evidence when a later
-- database time observation is behind the locked assignment's audit time.

create or replace function vortex_access.coordinate_assignment_change_without_stewardship_v1_internal(
  p_operation text,
...
comment on function vortex_access.coordinate_assignment_change_without_stewardship_v1_internal(
  text, uuid, uuid, bigint, uuid, bigint, text, uuid, uuid, text,
  timestamptz, timestamptz, uuid, uuid
) is
  'Private atomic role-assignment fact and Access change used by the stewardship-preserving public composition.';
