-- #1070: display-only Access changes. Creating an empty Group, renaming a
-- Group and revising a role's label or description are single
-- revision-checked writes that leave the organisation Access version, and
-- every cached decision, untouched. Group retirement and every membership,
-- assignment, activation, delegation and permission change still advance it.
--
-- Each function below is its complete definition, identical to its canonical
-- file under supabase/schemas/vortex_access. The Group coordinator is its live
-- body from 20260923050000 with only the retire-only increment changed. The
-- role-metadata request entry keeps every fixed protected check of the
-- 20260912141932 entry and appends one revision that carries every other role
-- fact forward (permissions, classification, assignment and activation policy,
-- continuity revisions and source evidence), so it cannot change authority;
-- its separate preparation entry and prepared-evidence argument are removed.
-- All three functions are owned by postgres, the migration role.
...
begin
  ...
  -- Creating an empty Group or revising a Group label changes no
  -- membership, assignment or permission fact, so it leaves the Access
  -- version (and every cached decision) untouched; retirement still advances it.
  if p_operation = 'retire_group' then
    select version.current_version
    ...
    next_access_version := locked_access_version;
  end if;
...
comment on function vortex_access.coordinate_organization_group_change(
  ...
  'Owner-only atomic Group creation, label revision or terminal retirement. Only retirement changes Access, once; creation and label revision leave the Access version unchanged. It grants no caller authority.';

drop function vortex_access.prepare_organization_role_metadata_change_for_administration(
  uuid, bigint, uuid
);
drop function vortex_access.revise_organization_role_metadata_for_administration(
