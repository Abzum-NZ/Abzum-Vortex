-- Account offboarding inventory extension (#563).
-- Extends protected account-offboarding inventory across organisation-shared
-- and all retained ownership scopes (active, soft-deleted, removal_pending)
-- with per-record disclosure and affected applications.
...
begin
  ...
  loop
    ...
    select coalesce(pg_catalog.jsonb_agg(distinct binding.application_root_id order by binding.application_root_id), '[]'::jsonb)
    into contract_affected_apps
    from vortex_module.installation_bindings as binding
    where binding.organization_id = organization_id_value
      and catalogue_row.storage_contract_id = any (binding.storage_contract_ids)
    ...
    loop
      ...
      item_value := pg_catalog.jsonb_build_object(
        ...
        'affectedApplications', contract_affected_apps
      );
  ...
  return pg_catalog.jsonb_build_object(
    ...
    'affectedApplications', coalesce((
      select pg_catalog.jsonb_agg(distinct app_val.value order by app_val.value)
      from pg_catalog.jsonb_each(counts) as entry(key, value),
      lateral pg_catalog.jsonb_array_elements_text(entry.value -> 'affectedApplications') as app_val(value)
    ), '[]'::jsonb)
...
comment on function vortex_record.list_offboarding_owned_shared_records_internal(
  ...
  'Private organisation-shared account-offboarding inventory; denied candidate records are omitted without identity or count disclosure.';

-- Update the protected entry to support organization_shared sections alongside
-- application sections, lifting the temporary shared_transfer_authority_undecided gate.
...
begin
  ...
  -- Resolved #563: shared_transfer_authority_undecided gate is lifted;
  -- organization_shared scope is fully inventoried with affected applications.
...
comment on function vortex_record.list_offboarding_owned_records(
  ...
  'Protected account-offboarding inventory: accounts.manage once and per-record exact transfer disclosure for application and organization-shared scopes.';
