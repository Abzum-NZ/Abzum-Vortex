-- Accept the compiled record-ownership value `team` (#401, first slice of #45).
--
-- The compiler emits `team` for a Group-owned record type: the canonical wire
-- value is written by `writeModuleRecordOwnershipModeV1`
-- (contracts/src/record-ownership-compatibility.ts), carried by
-- `moduleRecordOwnershipModeV1Schema` (contracts/src/module-contracts.ts:986),
-- reused unchanged by Module V2 (contracts/src/module-contracts-v2.ts:814-818)
-- and inherited by V3. `group` is the current *runtime* term, never a compiled
-- wire value, and the compiler cannot emit it.
--
-- This provisioner accepted only `group`
-- (20260909101526_accept_module_v3_storage_pair.sql:151,180-186), so every
-- compiled Module release carrying a Group-owned record type was refused with
-- 42501 'Module record storage definition is invalid'. Only hand-built SQL
-- fixtures ever reached it, because they wrote the runtime term by hand; no
-- published release can contain `group`, so no stored catalogue row, fingerprint
-- or generated table changes here.
--
-- `team` is accepted *instead of*, not alongside, `group`: the compiler never
-- emits `group`, and carrying two spellings for one meaning invites exactly the
-- drift this defect is. Access already speaks the same compiled value --
-- `evaluate_current_record_ownership_visibility` and the exact record decision
-- both accept `none`, `organization_account`, `team`, `inherited`
-- (20260911101613_constrain_permission_record_scope_shape.sql:312,
-- 20260910040755_compose_exact_record_access_decision.sql:602) -- so this aligns
-- storage with both the compiler wire and the decision that reads the rows.
--
-- Exactly two lines change, both below: the accepted ownership values, and the
-- generated owner check's Group branch. The check it generates is unchanged in
-- meaning (a Group-owned row carries `owner_group_id` and no owner account).
-- Everything else is byte-identical to 20260909101526, and the function is
-- replaced under its existing owner, so signature, volatility, security mode,
-- empty search path, grants and ACL are unchanged, and no other object is
-- touched.
...
begin
  ...
  loop
    ...
    if not vortex_context.is_non_nil_uuid(storage_id::text)
      ...
      or ownership_mode_value not in ('none', 'organization_account', 'team', 'inherited')
