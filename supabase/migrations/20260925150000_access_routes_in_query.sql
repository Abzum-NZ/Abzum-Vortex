-- #1080: push the owner-account, owner-group and direct-share access routes into
-- run_module_query's scan predicate so a reader limited to some records gets full pages from a
-- large table.
--
-- The scan predicate only NARROWS candidate rows. Every row the scan returns still goes through
-- read_record, the exact per-row access decision, so a pushed-down predicate can never widen a
-- result. A route is pushed down only where its table form is exactly its per-row test; an
-- all-records route, an inherited-ownership chase, a relationship route or any failure leaves the
-- scan unrestricted. Each statement below is identical to the canonical file
