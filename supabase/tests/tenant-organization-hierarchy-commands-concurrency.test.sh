...
('$expiring_assignment_id','$tenant_id','$expiring_id',array['platform.tenant.organizations.rename'],now()-interval '1 hour',clock_timestamp()+interval '5 seconds',1,now()-interval '1 hour','$actor_id','b7${run_uuid:2}',now()-interval '1 hour','$actor_id','b7${run_uuid:2}');"
fixture_created=1
...
[ "$authority_expired" = yes ] || { echo 'database clock did not pass the queued authority expiry' >&2; exit 1; }
