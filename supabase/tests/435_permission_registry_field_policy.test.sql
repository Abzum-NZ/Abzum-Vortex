...
select has_column(
  'vortex_access', 'permission_catalogue_entries', 'field_policy',
  'catalogue entries preserve an optional Definition-owned field policy'
);
select col_type_is(
  'vortex_access', 'permission_catalogue_entries', 'field_policy', 'jsonb',
  'field policy uses lossless JSON storage'
);
select ok(
  vortex_access.permission_field_policy_is_valid(
    '{"readableFieldIds":["63400000-0000-4000-8000-000000000011","63400000-0000-4000-8000-000000000012"],"changeableFieldIds":["63400000-0000-4000-8000-000000000012"]}'::jsonb
  ),
  'the private validator accepts canonical unique arrays and a readable changeable subset'
);
select ok(
  vortex_access.permission_field_policy_is_valid(
    '{"readableFieldIds":["AAAAAAAA-0000-4000-8000-000000000011"],"changeableFieldIds":["aaaaaaaa-0000-4000-8000-000000000011"]}'::jsonb
  ),
  'field-policy identity and subset comparison normalize UUID case without rewriting bytes'
);
select ok(
  not vortex_access.permission_field_policy_is_valid(
    '{"readableFieldIds":["63400000-0000-4000-8000-000000000012","63400000-0000-4000-8000-000000000011"],"changeableFieldIds":[]}'::jsonb
  )
  and not vortex_access.permission_field_policy_is_valid(
    '{"readableFieldIds":["63400000-0000-4000-8000-000000000011"],"changeableFieldIds":["63400000-0000-4000-8000-000000000012"]}'::jsonb
  )
  and not vortex_access.permission_field_policy_is_valid(
    '{"readableFieldIds":["63400000-0000-4000-8000-000000000011","63400000-0000-4000-8000-000000000011"],"changeableFieldIds":[]}'::jsonb
  )
  and not vortex_access.permission_field_policy_is_valid(
    '{"readableFieldIds":["AAAAAAAA-0000-4000-8000-000000000011","aaaaaaaa-0000-4000-8000-000000000011"],"changeableFieldIds":[]}'::jsonb
  ),
  'the private validator refuses noncanonical, case-equivalent duplicate and non-subset policies'
);
...
as $function$
  select pg_catalog.jsonb_strip_nulls(pg_catalog.jsonb_build_object(
    ...
    'recordScope', p_scope,
    'fieldPolicy', case when p_scope is null then null else pg_catalog.jsonb_build_object(
      'readableFieldIds', pg_catalog.jsonb_build_array(
        '63400000-0000-4000-8000-000000000011'::uuid,
        '63400000-0000-4000-8000-000000000012'::uuid
      ),
...
select is(
  ...
  null::jsonb,
  'legacy storage preserves absent field policy as SQL null'
);
select is(
  (
    select record_scope
...
select throws_ok(
  pg_catalog.format(
    ...
    pg_catalog.jsonb_set(
      (select scoped from scope_candidates),
      '{entries,0,permission,fieldPolicy,changeableFieldIds}',
      '["63400000-0000-4000-8000-000000000013"]'::jsonb
    )::text,
    '93400000-0000-4000-8000-000000000001',
    '73400000-0000-4000-8000-000000000009'
  ),
  '22023',
  'Application permission entry is invalid',
  'registration refuses a changeable field outside the same permission readable set'
);
select is(
  (
    select current_version
  ...
  2::bigint,
  'field-policy validation refusal leaves Access unchanged'
);
...
select is(
  ...
  '{"readableFieldIds":["63400000-0000-4000-8000-000000000011","63400000-0000-4000-8000-000000000012"],"changeableFieldIds":["63400000-0000-4000-8000-000000000012"]}'::jsonb,
  'the canonical field policy is stored without loss'
);
select is(
  (
    select record_scope
...
select is(
  ...
  '{"readableFieldIds":["63400000-0000-4000-8000-000000000011","63400000-0000-4000-8000-000000000012"],"changeableFieldIds":["63400000-0000-4000-8000-000000000012"]}'::jsonb,
  'the private reader returns the exact stored field policy'
);
select is(
  vortex_access.application_permission_registration_matches_candidate(
    '23400000-0000-4000-8000-000000000001',
...
select throws_ok(
  ...
  '23514',
  null,
  'storage refuses field policy on a permission without a record type'
);
select is(
  (
    select current_version
  ...
  5::bigint,
  'field-policy storage refusal leaves Access unchanged'
);
